...
- `REQ-R-007:` When no suitable target session exists, Pixel Forge may use Agent Deck session-creation tooling to create a new isolated clone-backed session in the intended workspace and then bind directly to that created session.
...
- `REQ-S-006:` One Live Editor thread must have one default writable workspace root. Unless the operator explicitly retargets or loads a different mirror candidate, request packs, default direct edits, and default self-edit mirror launch should all follow that same bound workspace.
...
- `REQ-E-011:` The primary mirror-launch control must deterministically relaunch the latest mirror for the effective isolated workspace source instead of browsing project-wide mirror-history choices from the toolbar.
...
- `[validated]` New Pixel Forge-created Live Editor sessions now default to isolated Agent Deck clone workspaces under the project `.agents/` tree, while keeping the canonical project root as project identity. Basis: the Agent Deck bridge now creates clone-backed sessions by default, thread/session state persists the bound workspace path, project session discovery accepts clone workspaces under `.agents/`, and Live Editor request packs now write into that bound workspace instead of assuming the repo root.
...
- `[validated]` Fresh nested Pixel Forge mirrors now surface the session-target affordance even before a live-editor session is bound. Basis: the chat input always renders the session picker control, shows the unbound state explicitly, and exposes direct isolated-session creation instead of hiding the control until binding already exists.
