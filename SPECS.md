...
- `REQ-E-009:` A sibling Pixel Forge target must preserve controller UI fidelity for front-end inspection. Safety belongs to the isolated runtime/state boundary, not to stripping away the very UI being inspected.
- `REQ-E-010:` Target runtimes must still be able to surface the shared workspace selector and launch deeper sibling Pixel Forge targets so self-edit recursion remains possible when needed.
...
- `[validated]` Target runtimes now expose the same workspace-selector surface and recursive self-launch affordances instead of hiding them behind target-only front-end restrictions. Basis: target mode now mirrors the startup selector state, keeps a selector opener in-app, and no longer suppresses `Run Pixel Forge` inside the target runtime.
