...
- `[active]` Mirror runtimes are now the default self-edit path, but they still do not inherit full shell capability when Pixel Forge is running inside Pixel Forge.
- Why it is the limiter: startup/layout fidelity is much better once the mirror build replaces the dev target, but recursive self-development still breaks down if the inner Pixel Forge cannot own nested preview surfaces and shell-level affordances inside the preview it lives in.
- Smallest complete unit to attack it: give mirror runtimes loaded inside preview a scoped desktop bridge with nested preview ownership, bounds translation, and event routing so the inner Pixel Forge can operate as a real shell-grade self-edit surface.
- Immediate proof target: from the controller, launch a mirror target, open Live Editor inside that mirror, launch a deeper Pixel Forge mirror from there, and confirm both levels can preview/select/update without UI divergence or preview-surface confusion.
...
- `[validated]` The default self-edit launch path now builds an isolated mirror runtime instead of defaulting to the dev/HMR target lane. Basis: the local-target launcher now starts `runtime_kind=mirror`, builds the workspace frontend into an isolated state-dir dist, serves it through the sibling backend, and runtime smoke returned `http://pixel-forge-mirror-target-...localhost` with no Vite/HMR server in the log.
