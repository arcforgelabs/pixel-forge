...
- Fresh Live Editor chats now start as unbound drafts. They carry only intended agent state until the first real bind. Clone-backed first bind under `.agents/` remains the default, but root-backed first bind is already a real backend mode and should become an explicit operator-facing choice instead of hidden API knowledge.
...
- Live Editor handoff still has bootstrap-once and delta-after continuity, but the visible transport now stays prompt-first: the operator prompt remains intact and Pixel Forge adds only compact refs for the current turn bundle.
...
- Explicit slash-skill requests are promoted out of freeform user prose into `turn-input.json`, a dedicated request-pack `## Skills` section, and compact dispatch refs as exact skill names instead of optional hints buried in wrapper prose.
...
- Later dispatches into that same session send the new operator prompt plus compact refs to the new turn bundle and mirrors while reusing the same stable thread brief behind the scenes.
