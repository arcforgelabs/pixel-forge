...
- The sidebar is now intended to be a pure project-scoped projection over the shared chat/session registry, not a reconciliation engine of its own. Every persisted chat/session/lane carries explicit owning `projectPath`, and frontend merge paths must update only that owning project's registry slices instead of inferring ownership from whichever project is currently selected.
- In the current frontend shape, flat `projectChats` / `projectSessions` collections are only active-project convenience projections. They should continue to converge toward selectors over the project-scoped registry rather than becoming a second ownership truth beside `projectChatsByProject` / `projectSessionsByProject`.
...
- The dispatcher chooses the adapter from the chat's selected agent and then stays inside that adapter for the whole turn. Claude logic must not leak into Codex paths, Codex logic must not leak into Claude paths, and future agents like Gemini, Pi, or OpenCode must arrive by registering a new adapter instead of editing mixed conditionals across the shared lane.
