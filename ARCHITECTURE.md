...
- The Projects sidebar now renders one reconciled Pixel Forge chat list per project. Persisted lanes remain authoritative, visible Agent Deck sessions are reconciliation inputs, and unmatched live sessions are adopted into chat rows instead of being rendered as a separate raw-session class.
