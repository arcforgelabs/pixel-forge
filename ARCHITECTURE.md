...
- **Restart/resume path (proven, current):** Agent Deck's `transcript_sync.go` watches JSONL via fsnotify, detects idle pane with empty composer, and triggers `Restart()` which does `tmux respawn-pane` + `claude --resume <sid>`. The new process re-reads the full JSONL and renders everything. Tradeoff: clears operator input, 1-2s cold restart.
