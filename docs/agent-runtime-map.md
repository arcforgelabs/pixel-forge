...
- the repo now also carries a bundled bootstrap helper: `scripts/bootstrap-claude-channel-spike.sh` installs the plugin, registers the local Arc Forge marketplace, writes a persisted shared-state env file, and `scripts/agent-deck.sh` auto-sources that env on future launches
