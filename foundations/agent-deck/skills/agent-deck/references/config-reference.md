...
```toml
[codex]
yolo_mode = true   # Enable Codex bypass mode
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `yolo_mode` | bool | `false` | Maps to `codex --dangerously-bypass-approvals-and-sandbox`. Can be overridden per-session. |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `default_provider` | string | `xai` | Provider used for new Pi sessions when not overridden per session. |
| `default_model` | string | `grok-4` | Model used for new Pi sessions when not overridden per session. |
| `default_thinking` | string | `""` | Thinking preset for new Pi sessions. |
| `env_file` | string | `""` | A .env file sourced for Pi sessions only. Sourced after global `[shell].env_files`. |

## [worktree] Section

Git worktree defaults for isolated sessions.

```toml
[worktree]
auto_cleanup = true
default_location = "subdirectory"
# path_template = "~/worktrees/{repo-name}/{branch-escaped}"
# branch_prefix = "feature/"
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `auto_cleanup` | bool | `true` | Remove worktree directories when their sessions are deleted/finished. |
| `default_location` | string | `subdirectory` | Worktree placement strategy: `sibling`, `subdirectory`, or custom path root. |
| `path_template` | string | `""` | Optional path template overriding `default_location`. Supports `{repo-name}`, `{repo-root}`, `{session-id}`, `{branch}`, and `{branch-escaped}`. |
| `branch_prefix` | string | `feature/` | Prefix used for auto-generated worktree branch names in the TUI. |

## [clone] Section

Reference clone defaults for stronger workspace isolation.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `auto_cleanup` | bool | `true` | Remove clone directories when their sessions are deleted/finished. |
| `dissociate` | bool | `false` | Use `git clone --dissociate` so the clone copies objects instead of sharing alternates. |
| `symlink_node_modules` | bool | `true` | Symlink the source repo's `node_modules` into the clone when present. The managed symlink itself does not make the clone dirty. |
| `copy_env_files` | bool | `true` | Copy `.env*` files from the source repo into the clone. |
...
**Built-in icons:** claude=🤖, gemini=✨, opencode=🌐, codex=💻, pi=π, cursor=📝, shell=🐚
