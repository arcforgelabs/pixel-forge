...
Conductors are persistent sessions (usually `conductor-<profile>`) that orchestrate work in a profile. This fork now keeps the bridge intentionally minimal:

- No `agent-deck conductor ...` CLI command surface in Agent Deck
- Telegram-only bridge transport (Slack removed)
- Transport-only behavior: receive signal -> wrap envelope -> forward -> relay response/error
- No bridge-owned heartbeat triage/escalation/parked/ownership decision engines

Conductor assets are generated into `~/.agent-deck/conductor/` from embedded templates when conductor bootstrap runs. Start conductor sessions normally:

```bash
agent-deck -p <profile> session start conductor-<profile>
```

Run bridge runtime from the generated install path:

```bash
python3 ~/.agent-deck/conductor/bridge.py
```

Each conductor gets its own directory and identity files:

```
~/.agent-deck/conductor/
├── CLAUDE.md           # Shared knowledge (CLI ref, protocols, rules)
├── bridge.py           # Bridge daemon (Telegram transport)
├── ops/
...
The Telegram bridge routes messages to specific conductors using a `name: message` prefix:
...
**Local fork / layered runtime**
```bash
./dev-install.sh
agent-deck-layers status
```

`./dev-install.sh` is the fork workflow: it builds the current checkout, publishes a `workstation` runtime layer under `~/.local/share/agent-deck/layers/`, and activates that layer behind `~/.local/bin/agent-deck`.

**Stock upstream runtime side-by-side**
```bash
...
| [Architecture](docs/architecture.md) | Current fork architecture, boundaries, and target long-term shape |
