...
configure_tmux() {
    ...
    if [[ -f "$TMUX_CONF" ]] && grep -q "$MARKER" "$TMUX_CONF" 2>/dev/null; then
        ...
        if [[ -z "$INSTALLED_VERSION" || "$INSTALLED_VERSION" -lt "$CURRENT_VERSION" ]]; then
            ...
            echo "  • Disabled duplicate clipboard export with drag-to-copy"
            echo "  • Routed copy-mode copy actions through the external clipboard tool"
...
# Disable tmux OSC 52 clipboard export here because drag-to-copy already uses
# an external clipboard tool. Leaving both enabled can double-fire clipboard
# updates in terminals with clipboard support such as Ghostty.
set -s set-clipboard off
set -s copy-command \"$CLIPBOARD_CMD\"
