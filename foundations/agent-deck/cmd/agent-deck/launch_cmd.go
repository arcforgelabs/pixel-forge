...
func handleLaunch(profile string, args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Println("  agent-deck launch . -c codex --yolo")
	...
	// Validate --clone and --worktree are mutually exclusive
	if *cloneName != "" && wtBranch != "" {
		out.Error("--clone and --worktree cannot be used together", ErrCodeInvalidOperation)
		os.Exit(1)
	}
	...
	if *cloneName != "" {
		...
		if err != nil {
			out.Error(fmt.Sprintf("failed to create clone: %v", err), ErrCodeInvalidOperation)
			os.Exit(1)
		}
