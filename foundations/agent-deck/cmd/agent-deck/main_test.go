...
func TestNestedSessionAllowsCLICommands(t *testing.T) {
	...
	t.Run("subcommands_dispatched_before_nested_check", func(t *testing.T) {
		...
		subcommands := []string{
			...
			"session", "mcp", "skill", "group", "try", "worktree", "wt", "clone",
...
func TestReorderArgsForFlagParsing_CloneFlagValuePreserved(t *testing.T) {
	args := []string{"--clone", "agent-reviewer", ".", "-c", "claude"}
	got := reorderArgsForFlagParsing(args)

	want := []string{"--clone", "agent-reviewer", "-c", "claude", "."}
	if len(got) != len(want) {
		t.Fatalf("reorderArgsForFlagParsing() len = %d, want %d (%v)", len(got), len(want), got)
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("reorderArgsForFlagParsing()[%d] = %q, want %q (full=%v)", i, got[i], want[i], got)
		}
	}
}
...
func TestDetectTool(t *testing.T) {
	tests := []struct {
		...
		{input: "pi", want: "pi"},
		{input: "pi --provider xai", want: "pi"},
		{input: "openclaw tui", want: "openclaw"},
		{input: "codex", want: "codex"},
		{input: "pipeline-check", want: "shell"},
	}

	for _, tt := range tests {
		if got := detectTool(tt.input); got != tt.want {
			t.Errorf("detectTool(%q) = %q, want %q", tt.input, got, tt.want)
		}
	}
}
