...
// Fork versioning convention: <upstream-version>-fork.
const Version = "0.26.0-fork"

var DisplayVersion = buildDisplayVersion(Version)

func buildDisplayVersion(baseVersion string) string {
	...
	for _, setting := range info.Settings {
		...
		case "vcs.revision":
			if len(setting.Value) >= 8 {
				shortSHA = setting.Value[:8]
			} else {
				shortSHA = setting.Value
			}
		case "vcs.modified":
...
func main() {
	...
	if len(args) > 0 {
		...
		case "version", "--version", "-v":
			fmt.Printf("%s v%s\n", ui.RuntimeDisplayName(), DisplayVersion)
			return
		case "help", "--help", "-h":
			printHelp()
			return
...
func handleAdd(profile string, args []string) {
	...
	cloneName := fs.String("clone", "", "Create session in reference clone (.agents/<name>)")
	dissociate := fs.Bool("dissociate", false, "Copy objects instead of sharing (use with --clone)")
	...
	fs.Usage = func() {
		...
		fmt.Println()
		fmt.Println("Clone Examples:")
		fmt.Println("  agent-deck add --clone reviewer-1 .")
		fmt.Println("  agent-deck add --clone reviewer-2 --dissociate .")
		fmt.Println()
	...
	if *cloneName != "" && wtBranch != "" {
		fmt.Println("Error: --clone and --worktree cannot be used together")
		os.Exit(1)
	}
	...
	var clonePath, cloneRepoRoot, cloneBranch string
	if *cloneName != "" {
		if !git.IsGitRepo(path) {
			fmt.Fprintf(os.Stderr, "Error: %s is not a git repository\n", path)
			os.Exit(1)
		...
		useDissociate := *dissociate || session.GetCloneSettings().Dissociate
		clonedPath, err := git.ReferenceClone(repoRoot, *cloneName, "", useDissociate)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: failed to create clone: %v\n", err)
			os.Exit(1)
		}

		cloneSettings := session.GetCloneSettings()
		if cloneSettings.SymlinkNodeModules || cloneSettings.CopyEnvFiles {
			if err := git.SetupCloneExtras(clonedPath, repoRoot, git.CloneExtrasOptions{
				SymlinkNodeModules: cloneSettings.SymlinkNodeModules,
				CopyEnvFiles:       cloneSettings.CopyEnvFiles,
			}); err != nil {
				fmt.Fprintf(os.Stderr, "Warning: failed to set up clone extras: %v\n", err)
			}
		}

		clonePath = clonedPath
		cloneRepoRoot = repoRoot
		cloneBranch = "agent/" + *cloneName
		path = clonePath
	}
	...
	if clonePath != "" {
		newInstance.WorktreePath = clonePath
		newInstance.WorktreeRepoRoot = cloneRepoRoot
		newInstance.WorktreeBranch = cloneBranch
		newInstance.IsolationType = "clone"
	}
	...
	if clonePath != "" {
		humanLines = append(humanLines, fmt.Sprintf("  Clone:   %s (branch: %s)", clonePath, cloneBranch))
		humanLines = append(humanLines, fmt.Sprintf("  Repo:    %s", cloneRepoRoot))
	}
	if *sshHost != "" {
	...
	if clonePath != "" {
		jsonData["clone_path"] = clonePath
		jsonData["clone_branch"] = cloneBranch
		jsonData["clone_repo_root"] = cloneRepoRoot
		jsonData["isolation_type"] = "clone"
	}
	if *resumeSession != "" {
...
func checkCloneRemoveSafety(inst *session.Instance) error {
...
func handleRemove(profile string, args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Println("For clone-backed sessions with local work, use `agent-deck clone finish` instead.")
		fmt.Println()
		fmt.Println("Examples:")
	...
	if inst.IsClone() {
		if err := checkCloneRemoveSafety(inst); err != nil {
			out.Error(
				fmt.Sprintf(
					"cannot remove clone session '%s': %v. Use `agent-deck clone finish %q` to finish locally; it does not push to origin",
					inst.Title,
					err,
					inst.Title,
				),
	...
	// Clean up isolation workspace if this session owns one.
	if inst.IsWorktree() {
	...
	} else if inst.IsClone() {
		if err := git.RemoveClone(inst.WorktreePath, false); err != nil {
			out.Error(fmt.Sprintf("failed to remove clone for '%s': %v", inst.Title, err), ErrCodeInvalidOperation)
			os.Exit(1)
		}
	}
...
func handleUpdate(args []string) {
	...
	if err := update.UpdateBridgePy(); err != nil {
		...
		fmt.Println("  Re-publish/reinstall the active runtime if you need to refresh conductor assets.")
	}
...
func printHelp() {
	...
	fmt.Println("  clone            Manage reference clones")
	...
	fmt.Println()
	fmt.Println("Clone Commands:")
	fmt.Println("  clone list                List reference clones with session associations")
	fmt.Println("  clone info <session>      Show clone info for a session")
	fmt.Println("  clone checkpoint <session> Local integrate + resync for a clone session")
	fmt.Println("  clone resync <session>    Realign a clone session to the local target tip")
	fmt.Println("  clone finish <session>    Local merge/cleanup for a clone session (no push)")
	fmt.Println("  clone cleanup             Find and remove orphaned clones/sessions")
	fmt.Println()
	...
	fmt.Println("  W          Finish worktree/clone (local merge + cleanup)")
	fmt.Println("  I          Checkpoint clone (local integrate + resync, AI option in dialog)")
	fmt.Println("  A          AI closeout for one worktree/clone")
	fmt.Println("  B          AI repo sweep for all worktrees/clones under repo")
...
func detectTool(cmd string) string {
	...
	case strings.Contains(cmd, "codex"):
		return "codex"
	case cmd == "pi" || strings.HasPrefix(cmd, "pi "):
		return "pi"
	case strings.Contains(cmd, "openclaw"):
		return "openclaw"
