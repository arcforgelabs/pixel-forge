...
func TestIsForkVersion(t *testing.T) {
	tests := []struct {
		...
		{version: "0.20.2-fork", want: true},
		{version: "v0.20.2-fork", want: true},
		{version: "0.19.14-fork.1.1.0", want: true},
		{version: "0.19.19", want: false},
		{version: "v0.19.19", want: false},
	}

	for _, tt := range tests {
		assert.Equal(t, tt.want, IsForkVersion(tt.version), tt.version)
	}
}
...
func TestUpdateBridgePy_UsesEmbeddedTemplateAndBacksUpExistingFile(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, strings.Contains(string(newContent), "Conductor Bridge: Telegram ->"),
		"bridge.py should be refreshed from the embedded transport-only template")
}
...
func TestShouldIgnoreLatestVersion(t *testing.T) {
	tests := []struct {
		name    string
		latest  string
		ignored string
		...
		{name: "no ignored version", latest: "0.19.16", ignored: "", want: false},
		{name: "exact ignored version", latest: "0.19.16", ignored: "0.19.16", want: true},
		{name: "latest newer than ignored", latest: "0.19.17", ignored: "0.19.16", want: false},
		{name: "latest older than ignored", latest: "0.19.15", ignored: "0.19.16", want: true},
		{name: "with v prefix", latest: "v0.19.16", ignored: "v0.19.16", want: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := shouldIgnoreLatestVersion(tt.latest, tt.ignored)
			assert.Equal(t, tt.want, got)
		})
	}
}

func TestCheckForUpdateHonorsIgnoredVersionInCache(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)

	originalInterval := checkInterval
	checkInterval = 24 * time.Hour
	t.Cleanup(func() { checkInterval = originalInterval })

	require.NoError(t, saveCache(&UpdateCache{
		CheckedAt:      time.Now(),
		LatestVersion:  "0.19.16",
		CurrentVersion: "0.19.14",
		DownloadURL:    "https://example.com/agent-deck.tar.gz",
		ReleaseURL:     "https://example.com/release",
		IgnoredVersion: "0.19.16",
	}))

	info, err := CheckForUpdate("0.19.14", false)
	require.NoError(t, err)
	require.NotNil(t, info)
	assert.Equal(t, "0.19.16", info.LatestVersion)
	assert.False(t, info.Available)
	assert.True(t, info.Ignored)

	// Newer release should re-notify automatically.
	require.NoError(t, saveCache(&UpdateCache{
		CheckedAt:      time.Now(),
		LatestVersion:  "0.19.17",
		CurrentVersion: "0.19.14",
		DownloadURL:    "https://example.com/agent-deck.tar.gz",
		ReleaseURL:     "https://example.com/release",
		IgnoredVersion: "0.19.16",
	...
	require.NoError(t, err)
	require.NotNil(t, info)
	assert.Equal(t, "0.19.17", info.LatestVersion)
	assert.True(t, info.Available)
	assert.False(t, info.Ignored)
}

func TestCheckForUpdate_SkipsForkBuilds(t *testing.T) {
	info, err := CheckForUpdate("0.20.2-fork", true)
	require.NoError(t, err)
	require.NotNil(t, info)
	assert.False(t, info.Available)
	assert.Equal(t, "0.20.2-fork", info.CurrentVersion)
	assert.Equal(t, "0.20.2-fork", info.LatestVersion)
}

func TestIgnoreAndClearIgnoredVersion(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)

	require.NoError(t, IgnoreVersion("v0.19.16"))
	cache, err := loadCache()
	require.NoError(t, err)
	assert.Equal(t, "0.19.16", cache.IgnoredVersion)

	require.NoError(t, ClearIgnoredVersion())
	cache, err = loadCache()
	require.NoError(t, err)
	assert.Equal(t, "", cache.IgnoredVersion)
}
