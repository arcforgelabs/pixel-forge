...
// IsForkVersion reports whether a version follows the fork convention:
// <upstream-version>-fork
func IsForkVersion(version string) bool {
	return strings.Contains(normalizeVersion(version), "-fork")
}

func forkBaseVersion(version string) string {
	v := normalizeVersion(version)
	if idx := strings.Index(v, "-fork"); idx > 0 {
		return v[:idx]
	}
	return v
}

func shouldIgnoreLatestVersion(latestVersion, ignoredVersion string) bool {
	latestVersion = normalizeVersion(latestVersion)
	ignoredVersion = normalizeVersion(ignoredVersion)
	if latestVersion == "" || ignoredVersion == "" {
		return false
	}
	// Ignore the explicitly muted release and any older release tags.
	return CompareVersions(latestVersion, ignoredVersion) <= 0
}
...
func CheckForUpdate(currentVersion string, forceCheck bool) (*UpdateInfo, error) {
	...
	// Fork builds are intentionally managed via selective upstream sync/cherry-pick,
	// not via upstream release replacement.
	if IsForkVersion(currentVersion) {
	...
	if !forceCheck {
		...
		if err == nil && time.Since(cache.CheckedAt) < checkInterval {
			...
			hasNewer := CompareVersions(currentVersion, cache.LatestVersion) < 0
			info.Ignored = hasNewer && shouldIgnoreLatestVersion(cache.LatestVersion, cache.IgnoredVersion)
			info.Available = hasNewer && !info.Ignored
			return info, nil
		}
	}
	...
	// Preserve ignored version marker across cache refreshes.
	if previousCache, cacheErr := loadCache(); cacheErr == nil {
		ignoredVersion = normalizeVersion(previousCache.IgnoredVersion)
	}
	...
	info.Ignored = hasNewer && shouldIgnoreLatestVersion(latestVersion, ignoredVersion)
	info.Available = hasNewer && !info.Ignored

	return info, nil
}

// IgnoreVersion suppresses notifications for a specific version until a newer
// version is released.
func IgnoreVersion(version string) error {
	...
	cache.IgnoredVersion = version
	return saveCache(cache)
}

// ClearIgnoredVersion removes the muted update marker.
func ClearIgnoredVersion() error {
	cache, err := loadCache()
	if err != nil {
		// No cache yet means nothing is muted.
		return nil
	}
	cache.IgnoredVersion = ""
	return saveCache(cache)
}
