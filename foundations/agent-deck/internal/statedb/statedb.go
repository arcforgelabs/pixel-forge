...
func (s *StateDB) Migrate() error {
	...
	case existingVersion != schemaVersion:
		// v1 → v2: Add isolation_type column for reference clone support
		if existingVersion == "1" {
			if _, err := tx.Exec(`ALTER TABLE instances ADD COLUMN isolation_type TEXT NOT NULL DEFAULT ''`); err != nil {
				// Column might already exist (concurrent migration), ignore
				if !strings.Contains(err.Error(), "duplicate column") {
					return fmt.Errorf("statedb: add isolation_type: %w", err)
				}
			}
		}
		if _, err := tx.Exec(`
...
func (s *StateDB) SaveInstance(inst *InstanceRow) error {
	...
	`,
		...
		inst.IsolationType, string(toolData),
	)
	return err
}
