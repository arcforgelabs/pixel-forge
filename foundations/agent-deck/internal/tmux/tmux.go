...
func parseComposerPromptLineForRetry(line string) (string, bool) {
	trimmed := strings.TrimSpace(StripANSI(line))
	if !strings.HasPrefix(trimmed, "❯") {
		return "", false
	}
	promptText := strings.TrimSpace(strings.TrimPrefix(trimmed, "❯"))
	if promptText == "" {
		return "", false
	}
...
func hasBypassIndicatorBelowPrompt(lines []string, promptIdx int) bool {
	limit := promptIdx + 4
	if limit > len(lines) {
		limit = len(lines)
	}
	for _, line := range lines[promptIdx+1 : limit] {
		trimmed := strings.ToLower(strings.TrimSpace(StripANSI(line)))
		if strings.Contains(trimmed, "bypass permissions on") ||
			strings.Contains(trimmed, "[tab] for shortcuts") {
			return true
		}
	}
	return false
...
func normalizePromptTextForRetry(text string) string {
	text = strings.ToLower(StripANSI(text))
...
func promptLikelyMatchesSentMessage(promptText, sentMessage string) bool {
	prompt := normalizePromptTextForRetry(promptText)
	sent := normalizePromptTextForRetry(sentMessage)
	if prompt == "" || sent == "" {
		return false
	}
	if strings.Contains(prompt, sent) || strings.HasPrefix(sent, prompt) {
		return true
	}
	needle := sent
	if len(needle) > 32 {
	...
	return strings.Contains(prompt, needle)
}

func (s *Session) detectStuckComposerPrompt(content, sentMessage, previousHash string) (bool, string, string) {
	lines := strings.Split(content, "\n")
	start := 0
	if len(lines) > 60 {
		start = len(lines) - 60
	...
	promptText := ""
	for i := len(lines) - 1; i >= start; i-- {
		text, ok := parseComposerPromptLineForRetry(lines[i])
		if !ok {
			continue
		}
		promptIdx = i
	...
	if promptIdx < 0 || !hasBypassIndicatorBelowPrompt(lines, promptIdx) {
		return false, "", ""
	}
	if !promptLikelyMatchesSentMessage(promptText, sentMessage) {
		return false, "", ""
	}

	currentHash := s.hashContent(s.normalizeContent(content))
	if previousHash == "" || currentHash != previousHash {
		return false, currentHash, promptText
	}
	return true, currentHash, promptText
}
...
// SendSpecialKeys sends one or more tmux key names (for example "Down",
// "Enter", "Escape") to the session.
func (s *Session) SendSpecialKeys(keys ...string) error {
	s.invalidateCache()
	if len(keys) == 0 {
		return nil
	}
