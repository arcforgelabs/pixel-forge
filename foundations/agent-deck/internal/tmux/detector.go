...
func (d *PromptDetector) HasPrompt(content string) bool {
	...
	case "codex":
		return d.hasCodexPrompt(content)
...
func (d *PromptDetector) hasCodexPrompt(content string) bool {
	...
	for i := len(lines) - 1; i >= 0 && len(lastLines) < 6; i-- {
	...
	for _, line := range lastLines {
		...
		if clean == "codex>" || strings.HasPrefix(clean, "codex> ") {
			return true
		}
		if clean == "Continue?" || strings.Contains(clean, "Continue?") {
			return true
		}
