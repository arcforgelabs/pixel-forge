...
type UserConfig struct {
	...
	// Pi defines Pi Coding Agent integration settings
	Pi PiSettings `toml:"pi"`

	// Worktree defines git worktree preferences
	Worktree WorktreeSettings `toml:"worktree"`

	// Clone defines reference clone preferences
	Clone CloneSettings `toml:"clone"`
...
type CodexSettings struct {
	// YoloMode enables the Codex bypass flag for sessions
	// (bypass approvals and sandbox)
	// Default: false
	YoloMode bool `toml:"yolo_mode"`
}

// PiSettings defines Pi Coding Agent configuration.
type PiSettings struct {
	// DefaultProvider is the provider to use for new Pi sessions (e.g. "xai").
	// If empty, Pi uses the built-in default.
	DefaultProvider string `toml:"default_provider"`

	// DefaultModel is the model to use for new Pi sessions.
	// If empty, Pi uses the built-in default.
	DefaultModel string `toml:"default_model"`

	// DefaultThinking is the thinking preset for new Pi sessions.
	DefaultThinking string `toml:"default_thinking"`

	// EnvFile is a .env file specific to Pi sessions.
	// Sourced AFTER global [shell].env_files.
	EnvFile string `toml:"env_file"`
}
...
// CloneSettings defines reference clone preferences.
type CloneSettings struct {
	// AutoCleanup removes clone directory when session is deleted.
	AutoCleanup bool `toml:"auto_cleanup"`

	// Dissociate copies objects from the reference so the clone becomes independent.
	Dissociate bool `toml:"dissociate"`

	// SymlinkNodeModules creates a symlink to the source repo's node_modules.
	SymlinkNodeModules bool `toml:"symlink_node_modules"`

	// CopyEnvFiles copies .env* files from the source repo.
	CopyEnvFiles bool `toml:"copy_env_files"`
}
...
func isUserConfigFieldDefined(path ...string) bool {
	userConfigCacheMu.RLock()
	defer userConfigCacheMu.RUnlock()
	if userConfigMeta == nil {
		return false
	}
	return userConfigMeta.IsDefined(path...)
}
...
// GetCloneSettings returns clone settings from the user config.
// Returns sensible defaults if no config file exists.
func GetCloneSettings() CloneSettings {
	config, err := LoadUserConfig()
	if err != nil || config == nil {
	...
	if !isUserConfigFieldDefined("clone") {
		settings.AutoCleanup = true
		settings.SymlinkNodeModules = true
		settings.CopyEnvFiles = true
		return settings
	}
	if !isUserConfigFieldDefined("clone", "auto_cleanup") {
		settings.AutoCleanup = true
	}
	if !isUserConfigFieldDefined("clone", "symlink_node_modules") {
		settings.SymlinkNodeModules = true
	}
	if !isUserConfigFieldDefined("clone", "copy_env_files") {
		settings.CopyEnvFiles = true
	}
	return settings
}
...
# [codex]
# Enable Codex bypass mode by default (default: false)
