...
type Instance struct {
	...
	Ownership         string `json:"ownership,omitempty"`           // "user" or "cato" (heartbeat ownership contract)
...
func (inst *Instance) IsWorktree() bool {
	return inst.WorktreePath != "" && inst.IsolationType != "clone"
}

// IsClone returns true if this session is running in a reference clone.
func (inst *Instance) IsClone() bool {
	return inst.WorktreePath != "" && inst.IsolationType == "clone"
}

// IsIsolated returns true if this session has workspace isolation (worktree or clone).
func (inst *Instance) IsIsolated() bool {
...
func NewInstance(title, projectPath string) *Instance {
	...
	return &Instance{
		...
		Ownership:   OwnershipUser.String(),
		Tool:        "shell",
		Status:      StatusIdle,
		CreatedAt:   time.Now(),
...
func (i *Instance) resolveCodexOptions() *CodexOptions {
	opts := i.GetCodexOptions()
	if opts == nil {
		userConfig, _ := LoadUserConfig()
		return NewCodexOptions(userConfig)
	}

	resolved := *opts
	if resolved.YoloMode == nil {
		if config, err := LoadUserConfig(); err == nil && config != nil && config.Codex.YoloMode {
			yolo := true
			resolved.YoloMode = &yolo
		}
	}
	return &resolved
}
...
func (i *Instance) buildCodexCommand(baseCommand string) string {
	...
	if baseCommand == "codex" {
		...
		if i.CodexSessionID != "" {
			args = append(args, "resume", i.CodexSessionID)
			return envPrefix + docker.ShellJoinArgs(args)
		}
...
// buildPiCommand builds the command for Pi CLI.
// Pi has no stable per-session ID, so new sessions use configured defaults and
// restarts fall back to --continue via buildPiRestartCommand().
func (i *Instance) buildPiCommand(baseCommand string) string {
	...
	envPrefix := i.buildEnvSourceCommand()
	if strings.TrimSpace(baseCommand) != "pi" {
		return envPrefix + baseCommand
	}
	...
	if opts == nil {
		userConfig, _ := LoadUserConfig()
		opts = NewPiOptions(userConfig)
	}

	args := append([]string{"pi"}, opts.ToArgs()...)
	return envPrefix + docker.ShellJoinArgs(args)
}

func (i *Instance) buildPiRestartCommand() string {
	if i.Tool != "pi" {
		return i.Command
	}

	envPrefix := i.buildEnvSourceCommand()
	if strings.TrimSpace(i.Command) != "pi" {
		return envPrefix + i.Command
	}
	...
	restartOpts := *opts
	restartOpts.SessionMode = "continue"

	args := append([]string{"pi"}, restartOpts.ToArgs()...)
	return envPrefix + docker.ShellJoinArgs(args)
}
...
func (i *Instance) Restart() error {
	...
	// Pi has only previous-session resume, so restart by continuing the latest session.
	if i.Tool == "pi" && i.tmuxSession != nil && i.tmuxSession.Exists() {
		resumeCmd, containerName, err := i.prepareCommand(i.buildPiRestartCommand())
		if err != nil {
			return err
		}
		if containerName != "" {
		...
		if err := i.tmuxSession.RespawnPane(resumeCmd); err != nil {
			return fmt.Errorf("failed to restart Pi session: %w", err)
		}
	...
	if IsClaudeCompatible(i.Tool) && i.ClaudeSessionID != "" {
		...
		command = i.buildPiRestartCommand()
	} else {
...
func (i *Instance) CanRestart() bool {
	...
	// Pi sessions can always restart by continuing the previous Pi conversation.
	if i.Tool == "pi" {
		return true
	}
...
// GetPiOptions returns Pi-specific options, or nil if not set.
func (i *Instance) GetPiOptions() *PiOptions {
	if len(i.ToolOptionsJSON) == 0 {
		return nil
	}
	opts, err := UnmarshalPiOptions(i.ToolOptionsJSON)
	if err != nil {
		return nil
	}
	return opts
...
// SetPiOptions stores Pi-specific options.
func (i *Instance) SetPiOptions(opts *PiOptions) error {
	if opts == nil {
		i.ToolOptionsJSON = nil
		return nil
	}
