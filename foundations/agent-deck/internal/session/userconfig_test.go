...
func TestGetCloneSettings_ExplicitFalseValuesPreserved(t *testing.T) {
	tempDir := t.TempDir()
	originalHome := os.Getenv("HOME")
	os.Setenv("HOME", tempDir)
	defer os.Setenv("HOME", originalHome)
...
`
	...
	settings := GetCloneSettings()
	if settings.AutoCleanup {
		t.Fatal("AutoCleanup should stay false when explicitly configured")
	}
	if !settings.Dissociate {
		t.Fatal("Dissociate should stay true when explicitly configured")
	}
	if settings.SymlinkNodeModules {
		t.Fatal("SymlinkNodeModules should stay false when explicitly configured")
	}
	if settings.CopyEnvFiles {
		t.Fatal("CopyEnvFiles should stay false when explicitly configured")
	}
}
