...
func TestInstance_CanRestart_Pi(t *testing.T) {
	inst := &Instance{
		Tool:   "pi",
	...
	if !inst.CanRestart() {
		t.Error("CanRestart() should return true for Pi sessions")
	}
}

func TestBuildPiCommand_UsesPiOptions(t *testing.T) {
	...
	if !strings.Contains(cmd, "pi") {
		t.Fatalf("buildPiCommand() = %q, want pi command", cmd)
	}
	if !strings.Contains(cmd, "--provider xai") {
		t.Fatalf("buildPiCommand() = %q, want provider flag", cmd)
	}
	if !strings.Contains(cmd, "--model grok-4") {
		t.Fatalf("buildPiCommand() = %q, want model flag", cmd)
	}
	if !strings.Contains(cmd, "--thinking high") {
		t.Fatalf("buildPiCommand() = %q, want thinking flag", cmd)
	}

	restartCmd := inst.buildPiRestartCommand()
	if !strings.Contains(restartCmd, "--continue") {
		t.Fatalf("buildPiRestartCommand() = %q, want continue flag", restartCmd)
	}
}
