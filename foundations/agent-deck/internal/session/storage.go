...
func (s *Storage) SaveWithGroups(instances []*Instance, groupTree *GroupTree) error {
	...
	for i, inst := range instances {
		...
		rows[i] = &statedb.InstanceRow{
			...
			IsolationType:   inst.IsolationType,
			ToolData:        toolData,
		}
	}
...
func (s *Storage) LoadLite() ([]*InstanceData, []*GroupData, error) {
	...
	dbRows, dbGroups, err = s.pruneDisabledConductorInventory(dbRows, dbGroups)
	if err != nil {
		return nil, nil, err
	}
...
func (s *Storage) pruneDisabledConductorInventory(
	instanceRows []*statedb.InstanceRow,
	groupRows []*statedb.GroupRow,
) ([]*statedb.InstanceRow, []*statedb.GroupRow, error) {
	settings := GetConductorSettings()
	if settings.Enabled {
		return instanceRows, groupRows, nil
	}

	filteredInstances := make([]*statedb.InstanceRow, 0, len(instanceRows))
	prunedInstances := 0
	for _, row := range instanceRows {
		if row == nil {
			continue
		}
		if shouldPruneDisabledConductorRow(row) {
			prunedInstances++
			continue
		}
		filteredInstances = append(filteredInstances, row)
	}

	filteredGroups := make([]*statedb.GroupRow, 0, len(groupRows))
	prunedGroups := 0
	for _, row := range groupRows {
		if row == nil {
			continue
		}
		if strings.EqualFold(strings.TrimSpace(row.Path), "conductor") {
			prunedGroups++
			continue
		}
		filteredGroups = append(filteredGroups, row)
	}

	if prunedInstances == 0 && prunedGroups == 0 {
		return instanceRows, groupRows, nil
	}

	if err := s.db.SaveInstances(filteredInstances); err != nil {
		return nil, nil, fmt.Errorf("failed to prune disabled conductor instances: %w", err)
	}
	if err := s.db.SaveGroups(filteredGroups); err != nil {
		return nil, nil, fmt.Errorf("failed to prune disabled conductor groups: %w", err)
	}
	_ = s.db.Touch()

	storageLog.Info(
		"disabled_conductor_inventory_pruned",
		slog.String("profile", s.profile),
		slog.Int("instances", prunedInstances),
		slog.Int("groups", prunedGroups),
	)

	return filteredInstances, filteredGroups, nil
}

func shouldPruneDisabledConductorRow(row *statedb.InstanceRow) bool {
	if row == nil {
		return false
	}
	if strings.EqualFold(strings.TrimSpace(row.GroupPath), "conductor") {
		return true
	}
	return IsConductorSessionTitle(row.Title)
}
...
func (s *Storage) convertToInstances(data *StorageData) ([]*Instance, []*GroupData, error) {
	...
	for i, instData := range data.Instances {
		...
		inst := &Instance{
			...
			GroupPath:          groupPath,
			Ownership:          NormalizeOwnership(instData.Ownership).String(),
			...
			IsolationType:      instData.IsolationType,
