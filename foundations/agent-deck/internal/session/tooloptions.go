...
type CodexOptions struct {
	// YoloMode enables the Codex bypass flag (bypass approvals and sandbox)
...
// PiOptions holds launch options for Pi Coding Agent sessions
type PiOptions struct {
	// SessionMode: "new" (default), "continue" (-c)
	SessionMode string `json:"session_mode,omitempty"`
	// Provider overrides the provider (e.g., "google", "openai", "anthropic")
	Provider string `json:"provider,omitempty"`
	// Model overrides the model (e.g., "gemini-2.5-pro", "openai/gpt-4o")
	Model string `json:"model,omitempty"`
...
// ToolName returns "pi"
func (o *PiOptions) ToolName() string {
	return "pi"
}

// ToArgs returns command-line arguments based on options
func (o *PiOptions) ToArgs() []string {
	var args []string

	if o.SessionMode == "continue" {
...
// NewPiOptions creates PiOptions with defaults from config.
// Falls back to xai/grok-4 when no config overrides are set.
func NewPiOptions(config *UserConfig) *PiOptions {
	opts := &PiOptions{
		SessionMode: "new",
		Provider:    "xai",
		Model:       "grok-4",
	...
	if config != nil {
		if config.Pi.DefaultProvider != "" {
			opts.Provider = config.Pi.DefaultProvider
		}
		if config.Pi.DefaultModel != "" {
			opts.Model = config.Pi.DefaultModel
		}
		if config.Pi.DefaultThinking != "" {
			opts.Thinking = config.Pi.DefaultThinking
		}
	}
	return opts
}

// UnmarshalPiOptions deserializes PiOptions from JSON wrapper
func UnmarshalPiOptions(data json.RawMessage) (*PiOptions, error) {
	if len(data) == 0 {
		return nil, nil
	}
