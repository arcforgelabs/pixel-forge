...
// IsAncestor reports whether ancestorRef is reachable from descendantRef.
func IsAncestor(repoDir, ancestorRef, descendantRef string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to compare refs %s and %s: %w", ancestorRef, descendantRef, err)
	}
	return false, nil
}
