...
const (
	...
	ConfirmForceDeleteCloneSession
	ConfirmCloseSession
	ConfirmDeleteGroup
	ConfirmQuitWithPool
...
type ConfirmDialog struct {
	...
	forceDeleteReason string // Explanation for destructive clone deletion.
...
// ShowForceDeleteCloneSession shows confirmation for discarding local clone work.
func (c *ConfirmDialog) ShowForceDeleteCloneSession(sessionID string, sessionName string, sandboxed bool, reason string) {
	c.visible = true
	c.confirmType = ConfirmForceDeleteCloneSession
	c.targetID = sessionID
	c.targetName = sessionName
	c.sandboxed = sandboxed
	c.forceDeleteReason = strings.TrimSpace(reason)
}
...
func (c *ConfirmDialog) View() string {
	...
	case ConfirmForceDeleteCloneSession:
		title = "⚠  Force Delete Clone?"
		warning = fmt.Sprintf("This will discard local clone work and delete:\n\n  \"%s\"", c.targetName)
		details = "• The clone directory will be removed even if local work remains\n• Meaningful clone-only work will be archived under local git refs before deletion when possible\n• This is local cleanup only; it does not push to origin\n• Use W / clone finish instead if you want to keep or merge the work"
		if c.forceDeleteReason != "" {
			details = "• Current block: " + c.forceDeleteReason + "\n" + details
		}
		if c.sandboxed {
			details += "\n• The Docker container will be removed"
		}
