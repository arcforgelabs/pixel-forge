...
const (
	...
	// Background status polling intervals.
	// Keep full sweeps while attached (Bubble Tea is paused during tea.Exec),
	// but back off on the dashboard so idle tmux polling does not stall
	// navigation when the user returns.
	backgroundStatusAttachedInterval  = 2 * time.Second
	backgroundStatusDashboardInterval = 10 * time.Second
...
type Home struct {
	...
	worktreeFinishDialog      *WorktreeFinishDialog      // For finishing isolated sessions (merge + cleanup)
	cloneCheckpointDialog     *CloneCheckpointDialog     // For checkpoint integrate + resync on clone sessions
	delegatedCheckpointDialog *DelegatedCheckpointDialog // For agent-managed checkpoint of one clone session
	delegatedFinishDialog     *DelegatedFinishDialog     // For agent-managed closeout of isolated sessions
	repoCloseoutDialog        *RepoCloseoutDialog        // For agent-managed repo-wide closeout sweeps
	...
	lastStatusReq       statusUpdateRequest      // Snapshot of the most recent viewport state from the UI thread
	lastStatusReqMu     sync.RWMutex             // Protects lastStatusReq across UI + worker goroutines
	...
	cloneBranchStateCache  map[string]git.CloneBranchState
...
type sessionMessageSentMsg struct {
	sessionID    string
	sessionTitle string
	err          error
}
...
type worktreeFinishResultMsg struct {
	...
	merged       bool
	isClone      bool
	cloneArchive git.CloneArchiveResult
	err          error
}

// cloneCheckpointResultMsg is sent when a clone checkpoint operation completes.
type cloneCheckpointResultMsg struct {
	sessionID         string
	sessionTitle      string
	targetBranch      string
	branchStateBefore git.CloneBranchState
	merged            bool
	resynced          bool
	err               error
}
...
type backgroundStatusMode int

const (
	backgroundStatusModeIncremental backgroundStatusMode = iota
	backgroundStatusModeFull
)
...
func NewHomeWithProfileAndMode(profile string) *Home {
	...
	h := &Home{
		...
		cloneCheckpointDialog:     NewCloneCheckpointDialog(),
		delegatedCheckpointDialog: NewDelegatedCheckpointDialog(),
		delegatedFinishDialog:     NewDelegatedFinishDialog(),
		repoCloseoutDialog:        NewRepoCloseoutDialog(),
		cursor:                    0,
...
func (h *Home) backgroundStatusMode() backgroundStatusMode {
	if h.isAttaching.Load() {
		return backgroundStatusModeFull
	}
	return backgroundStatusModeIncremental
}

func (h *Home) backgroundStatusInterval() time.Duration {
	if h.backgroundStatusMode() == backgroundStatusModeFull {
		return backgroundStatusAttachedInterval
	}
	return backgroundStatusDashboardInterval
}

func (h *Home) snapshotStatusUpdateRequest() statusUpdateRequest {
...
func (h *Home) setLastStatusUpdateRequest(req statusUpdateRequest) {
	reqCopy := statusUpdateRequest{
		viewOffset:    req.viewOffset,
		visibleHeight: req.visibleHeight,
		flatItemIDs:   append([]string(nil), req.flatItemIDs...),
	}
	h.lastStatusReqMu.Lock()
	h.lastStatusReq = reqCopy
	h.lastStatusReqMu.Unlock()
}

func (h *Home) getLastStatusUpdateRequest() statusUpdateRequest {
	h.lastStatusReqMu.RLock()
	defer h.lastStatusReqMu.RUnlock()
	return statusUpdateRequest{
		viewOffset:    h.lastStatusReq.viewOffset,
		visibleHeight: h.lastStatusReq.visibleHeight,
		flatItemIDs:   append([]string(nil), h.lastStatusReq.flatItemIDs...),
	}
}
...
// statusWorker runs in a background goroutine with an adaptive timer.
// Full status updates continue while the TUI is paused (tea.Exec), but the
// dashboard path backs off to cheaper incremental updates to avoid tmux churn.
func (h *Home) statusWorker() {
	defer close(h.statusWorkerDone)

	timer := time.NewTimer(h.backgroundStatusInterval())
	defer timer.Stop()
	...
	resetTimer := func() {
		...
		timer.Reset(h.backgroundStatusInterval())
	}
	...
	for {
		...
		case <-timer.C:
			switch h.backgroundStatusMode() {
			case backgroundStatusModeFull:
				// tea.Exec pauses Bubble Tea ticks, so keep the full scan here.
				h.backgroundStatusUpdate()
			default:
				// On the dashboard, reuse the cheaper incremental updater and
				// then sync the notification bar from the latest known state.
				req := h.getLastStatusUpdateRequest()
...
func (h *Home) triggerStatusUpdate() {
	req := h.snapshotStatusUpdateRequest()
	h.setLastStatusUpdateRequest(req)
...
func (h *Home) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case sessionCreatedMsg:
		...
		if msg.err != nil {
			...
			// Start fetching preview for the new session and optionally send
			// a deferred initial message now that the session is visible/persisted.
			cmds := []tea.Cmd{h.fetchPreview(msg.instance, msg.instance.ID, -1)}
			if strings.TrimSpace(msg.postCreateMessage) != "" {
				cmds = append(cmds, h.sendInitialMessageToSession(msg.instance, msg.postCreateMessage))
			}
			return h, tea.Batch(cmds...)
		}
		return h, nil

	case delegatedFinishRequestMsg:
		return h.handleDelegatedFinishRequest(msg)

	case cloneCheckpointRequestMsg:
		return h.handleCloneCheckpointRequest(msg)

	case openDelegatedCheckpointDialogMsg:
		return h.handleOpenDelegatedCheckpointDialog(msg)

	case delegatedCheckpointRequestMsg:
		return h.handleDelegatedCheckpointRequest(msg)

	case repoCloseoutRequestMsg:
		return h.handleRepoCloseoutRequest(msg)

	case sessionMessageSentMsg:
		if msg.err != nil {
			h.setError(fmt.Errorf("failed to send initial message to %q: %w", msg.sessionTitle, msg.err))
		}
		return h, nil
	...
	case sessionDeletedMsg:
		...
		if deletedInstance != nil {
			message := fmt.Sprintf("deleted '%s'", deletedInstance.Title)
			if msg.cloneArchive.HasArchive() {
				message += fmt.Sprintf(". Archived clone state at %s", strings.Join(msg.cloneArchive.Refs(), ", "))
			}
			if undoKey := h.actionKey(hotkeyUndoDelete); undoKey != "" {
				h.setError(fmt.Errorf("%s. %s to undo", message, undoKey))
			} else {
				h.setError(fmt.Errorf("%s", message))
			}
		}
		return h, nil
	...
	case previewDebounceMsg:
		...
		if inst != nil {
			...
			if inst.IsIsolated() && inst.WorktreePath != "" {
				...
				if needsCheck {
					cmds = append(cmds, h.fetchIsolationStatus(inst))
				}
			}
	...
	case worktreeDirtyCheckMsg:
		...
		if msg.err == nil {
			...
			h.cloneBranchStateCache[msg.sessionID] = msg.cloneState
			h.cloneTargetBranchCache[msg.sessionID] = msg.cloneTargetRef
		...
		if h.worktreeFinishDialog.IsVisible() && h.worktreeFinishDialog.GetSessionID() == msg.sessionID && msg.err == nil {
			...
			h.worktreeFinishDialog.SetCloneBranchStatus(msg.cloneState, msg.cloneTargetRef)
		}
		if h.cloneCheckpointDialog.IsVisible() && h.cloneCheckpointDialog.GetSessionID() == msg.sessionID && msg.err == nil {
			h.cloneCheckpointDialog.SetDirtyStatus(msg.isDirty)
			h.cloneCheckpointDialog.SetCloneBranchStatus(msg.cloneState, msg.cloneTargetRef)
		}
		return h, nil
	...
	case worktreeFinishResultMsg:
		...
		delete(h.cloneBranchStateCache, msg.sessionID)
		delete(h.cloneTargetBranchCache, msg.sessionID)
		...
		successLabel := "worktree"
		if msg.isClone {
			successLabel = "clone"
		}
		successMsg := fmt.Sprintf("Finished %s '%s' locally", successLabel, msg.sessionTitle)
		if msg.merged {
			successMsg += fmt.Sprintf(", merged into local %s", msg.targetBranch)
		}
		if msg.cloneArchive.HasArchive() {
			successMsg += fmt.Sprintf(". Archived discarded clone state at %s", strings.Join(msg.cloneArchive.Refs(), ", "))
		}
		h.setError(fmt.Errorf("%s", successMsg))
		return h, nil

	case cloneCheckpointResultMsg:
		if msg.err != nil {
			if h.cloneCheckpointDialog.IsVisible() {
				h.cloneCheckpointDialog.SetError(msg.err.Error())
			} else {
				h.setError(msg.err)
			}
			return h, nil
		...
		h.cloneCheckpointDialog.Hide()
		h.worktreeDirtyMu.Lock()
		h.worktreeDirtyCache[msg.sessionID] = false
		h.cloneBranchStateCache[msg.sessionID] = git.CloneBranchStateInSync
		h.cloneTargetBranchCache[msg.sessionID] = msg.targetBranch
		h.worktreeDirtyCacheTs[msg.sessionID] = time.Now()
		h.worktreeDirtyMu.Unlock()

		successMsg := fmt.Sprintf("Checkpointed clone '%s' locally", msg.sessionTitle)
		switch msg.branchStateBefore {
		case git.CloneBranchStateBehind:
			successMsg += fmt.Sprintf(", resynced to local %s", msg.targetBranch)
		case git.CloneBranchStateInSync:
			successMsg += fmt.Sprintf(", already in sync with local %s", msg.targetBranch)
		default:
			successMsg += fmt.Sprintf(", integrated into local %s and resynced", msg.targetBranch)
		}
		h.setError(fmt.Errorf("%s", successMsg))
		...
		h.instancesMu.RUnlock()
		if inst != nil {
			return h, h.fetchIsolationStatus(inst)
		}
		return h, nil
	...
	case tea.KeyMsg:
		...
		if h.cloneCheckpointDialog.IsVisible() {
			d, cmd := h.cloneCheckpointDialog.Update(msg)
			h.cloneCheckpointDialog = d
			return h, cmd
		}
		if h.delegatedCheckpointDialog.IsVisible() {
			d, cmd := h.delegatedCheckpointDialog.Update(msg)
			h.delegatedCheckpointDialog = d
			return h, cmd
		}
		if h.delegatedFinishDialog.IsVisible() {
			d, cmd := h.delegatedFinishDialog.Update(msg)
			h.delegatedFinishDialog = d
			return h, cmd
		}
		if h.repoCloseoutDialog.IsVisible() {
			d, cmd := h.repoCloseoutDialog.Update(msg)
			h.repoCloseoutDialog = d
			return h, cmd
		}
...
func (h *Home) handleNewDialogKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		// Get values including isolation settings.
		name, path, command, branchName, cloneName, worktreeEnabled, cloneEnabled := h.newDialog.GetValuesWithIsolation()
		groupPath := h.newDialog.GetSelectedGroup()
		claudeOpts := h.newDialog.GetClaudeOptions() // Get Claude options if applicable.

		// Resolve isolation target; actual creation runs in async command.
		var isolation sessionIsolationSpec
		if worktreeEnabled && branchName != "" {
			...
			isolation = sessionIsolationSpec{
				Mode:     "worktree",
				Path:     worktreePath,
				RepoRoot: repoRoot,
				Branch:   branchName,
		...
		if cloneEnabled && cloneName != "" {
			...
			isolation = sessionIsolationSpec{
				...
				RepoRoot:   repoRoot,
				CloneName:  cloneName,
				Dissociate: session.GetCloneSettings().Dissociate,
			}
		}
		...
		// Only non-isolated sessions may need interactive "create directory" confirmation.
		if isolation.Mode == "" {
		...
		return h, h.createSessionInGroupWithIsolationAndOptions(
			name,
			path,
			command,
			groupPath,
...
func (h *Home) handleMainKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "W", "shift+w":
		// Finish isolated session - local merge + cleanup for worktrees/clones
		if h.cursor < len(h.flatItems) {
			item := h.flatItems[h.cursor]
			if item.Type == session.ItemTypeSession && item.Session != nil {
				inst := item.Session
				if !inst.IsWorktree() && !inst.IsClone() {
					h.setError(fmt.Errorf("session '%s' is not a worktree or clone", inst.Title))
					return h, nil
				}
				...
				h.worktreeFinishDialog.ShowWithType(
					inst.ID,
					inst.Title,
					inst.WorktreeBranch,
	...
	case "I", "shift+i":
		// Checkpoint clone session - local integrate + resync, keep session open
		if h.cursor < len(h.flatItems) {
			item := h.flatItems[h.cursor]
			if item.Type == session.ItemTypeSession && item.Session != nil {
				inst := item.Session
				if !inst.IsClone() {
					h.setError(fmt.Errorf("session '%s' is not a clone session", inst.Title))
					return h, nil
				}
				defaultBranch := "main"
				if detected, err := git.GetDefaultBranch(inst.WorktreeRepoRoot); err == nil {
				...
				h.cloneCheckpointDialog.SetSize(h.width, h.height)
				h.cloneCheckpointDialog.Show(
					inst.ID,
					inst.Title,
					inst.WorktreeBranch,
					inst.WorktreeRepoRoot,
	...
	case "A", "shift+a":
		// AI closeout - spawn a root-level agent to decide how to integrate one isolated session
		if h.cursor < len(h.flatItems) {
			item := h.flatItems[h.cursor]
			if item.Type == session.ItemTypeSession && item.Session != nil {
				inst := item.Session
				...
				h.delegatedFinishDialog.SetSize(h.width, h.height)
				h.delegatedFinishDialog.Show(
					inst.ID,
					inst.Title,
					inst.WorktreeBranch,
					inst.WorktreeRepoRoot,
	...
	case "B", "shift+b":
		// AI repo sweep - spawn a root-level agent to close out isolated work across one repo root
		anchorSessionID, anchorLabel, groupPath, repoRoot := h.prepareRepoCloseoutDialogContext()
		defaultBranch := "main"
		trackedSessions := []repoCloseoutTrackedSession(nil)
		orphanClones := []git.CloneInfo(nil)
		if repoRoot != "" {
			if detected, err := git.GetDefaultBranch(repoRoot); err == nil {
				defaultBranch = detected
			}
			trackedSessions = h.collectRepoCloseoutSessions(repoRoot)
			orphanClones = h.collectRepoCloseoutOrphanClones(repoRoot)
		}

		h.repoCloseoutDialog.SetSize(h.width, h.height)
		h.repoCloseoutDialog.Show(
			anchorSessionID,
			anchorLabel,
			groupPath,
			repoRoot,
	...
	case "d":
		...
		if h.cursor < len(h.flatItems) {
			item := h.flatItems[h.cursor]
			if item.Type == session.ItemTypeSession && item.Session != nil {
				if item.Session.IsClone() {
					if err := checkCloneSessionRemovalSafety(item.Session); err != nil {
						h.confirmDialog.ShowForceDeleteCloneSession(item.Session.ID, item.Session.Title, item.Session.IsSandboxed(), err.Error())
						return h, nil
					}
				}
	...
	case "t":
		// Toggle session ownership between user and Cato.
		if h.cursor < len(h.flatItems) {
			item := h.flatItems[h.cursor]
			if item.Type == session.ItemTypeSession && item.Session != nil {
				next := item.Session.ToggleOwnership()
				h.saveInstances()
				h.setError(fmt.Errorf("ownership set to %s for '%s'", next.String(), item.Session.Title))
			}
		}
		return h, nil
...
func (h *Home) handleConfirmDialogKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	default:
		...
		case "y", "Y":
			...
			case ConfirmForceDeleteCloneSession:
				sessionID := h.confirmDialog.GetTargetID()
				if inst := h.getInstanceByID(sessionID); inst != nil {
					h.confirmDialog.Hide()
					return h, h.deleteSessionWithOptions(inst, true)
				}
...
type sessionIsolationSpec struct {
	Mode       string
	Path       string
	RepoRoot   string
	Branch     string
...
func (s sessionIsolationSpec) IsWorktree() bool {
	return s.Mode == "worktree" && s.Path != "" && s.RepoRoot != "" && s.Branch != ""
}

func (s sessionIsolationSpec) IsClone() bool {
	return s.Mode == "clone" && s.RepoRoot != "" && s.CloneName != ""
}

// createSessionInGroupWithIsolationAndOptions creates a new session with full options
// including isolation mode, YOLO mode, sandbox, and tool options.
func (h *Home) createSessionInGroupWithIsolationAndOptions(
	...
	return func() tea.Msg {
		...
		if isolation.IsWorktree() {
			...
			if existingPath, err := git.GetWorktreeForBranch(isolation.RepoRoot, isolation.Branch); err == nil && existingPath != "" {
				uiLog.Info("worktree_reuse", slog.String("branch", isolation.Branch), slog.String("path", existingPath))
				isolation.Path = existingPath
			} else {
				if err := os.MkdirAll(filepath.Dir(isolation.Path), 0o755); err != nil {
					return sessionCreatedMsg{err: fmt.Errorf("failed to create parent directory: %w", err)}
				}
				if err := git.CreateWorktree(isolation.RepoRoot, isolation.Path, isolation.Branch); err != nil {
		...
		if isolation.IsClone() {
			clonedPath, err := git.ReferenceClone(isolation.RepoRoot, isolation.CloneName, "", isolation.Dissociate)
			if err != nil {
				return sessionCreatedMsg{err: fmt.Errorf("failed to create clone: %w", err)}
			}

			cloneSettings := session.GetCloneSettings()
			if cloneSettings.SymlinkNodeModules || cloneSettings.CopyEnvFiles {
				if err := git.SetupCloneExtras(clonedPath, isolation.RepoRoot, git.CloneExtrasOptions{
					SymlinkNodeModules: cloneSettings.SymlinkNodeModules,
					CopyEnvFiles:       cloneSettings.CopyEnvFiles,
				}); err != nil {
					uiLog.Warn("clone_extras_failed", slog.String("error", err.Error()))
				}
			}

			isolation.Path = clonedPath
			isolation.Branch = "agent/" + isolation.CloneName
			path = clonedPath
		}
		...
		// Set isolation fields if provided.
		if isolation.IsWorktree() {
			inst.WorktreePath = isolation.Path
			inst.WorktreeRepoRoot = isolation.RepoRoot
			inst.WorktreeBranch = isolation.Branch
		}
		if isolation.IsClone() {
			inst.WorktreePath = isolation.Path
			inst.WorktreeRepoRoot = isolation.RepoRoot
		...
		if startErr != nil {
			...
			return sessionCreatedMsg{err: startErr}
		}
		uiLog.Info("session_create_succeeded", slog.String("id", inst.ID))
		return sessionCreatedMsg{
			instance:          inst,
			postCreateMessage: strings.TrimSpace(initialMessage),
		}
	}
}
...
func (h *Home) quickCreateSession() tea.Cmd {
	...
	return h.createSessionInGroupWithIsolationAndOptions(
...
func checkCloneSessionRemovalSafety(inst *session.Instance) error {
	if inst == nil || !inst.IsClone() {
		return nil
	}

	if err := git.CheckCloneRemovalSafety(inst.WorktreePath); err != nil {
		return err
	}

	if inst.WorktreePath == "" || inst.WorktreeBranch == "" || inst.WorktreeRepoRoot == "" {
		return nil
	}
	...
	if _, err := os.Stat(inst.WorktreePath); os.IsNotExist(err) {
		...
		return fmt.Errorf("failed to inspect clone path: %w", err)
	}

	targetBranch, err := git.GetDefaultBranch(inst.WorktreeRepoRoot)
	if err != nil || targetBranch == "" || targetBranch == inst.WorktreeBranch {
		return nil
	}

	merged, err := git.IsAncestor(inst.WorktreePath, inst.WorktreeBranch, targetBranch)
	if err != nil {
		return nil
	}
	if !merged {
		return fmt.Errorf("clone branch '%s' is not merged into local %s", inst.WorktreeBranch, targetBranch)
	}

	return nil
}

func (h *Home) cloneDeletionBlockedError(inst *session.Instance) error {
	if err := checkCloneSessionRemovalSafety(inst); err != nil {
		finishKey := h.actionKey(hotkeyWorktreeFinish)
		if finishKey == "" {
			finishKey = "W"
		}
		return fmt.Errorf(
			"cannot delete clone '%s': %v. Use %s or agent-deck clone finish %q to finish locally; it does not push to origin",
			inst.Title,
			err,
			finishKey,
			inst.Title,
...
func (h *Home) fetchIsolationStatus(inst *session.Instance) tea.Cmd {
	sid := inst.ID
	...
	if inst.IsClone() {
		...
		return func() tea.Msg {
			status, err := git.GetCloneStatus(repoRoot, wtPath, branch)
			return worktreeDirtyCheckMsg{
				sessionID:      sid,
				isDirty:        status.Dirty,
				cloneState:     status.BranchState,
				cloneTargetRef: status.TargetBranch,
				err:            err,
			}
		}
	}
...
func describeCloneBranchStateCompact(state git.CloneBranchState, targetBranch string) string {
	if targetBranch == "" {
		return ""
	}
	...
	switch state {
	case git.CloneBranchStateInSync:
		return fmt.Sprintf("in sync with local %s", targetBranch)
	case git.CloneBranchStateAhead:
		return fmt.Sprintf("ahead of local %s", targetBranch)
	case git.CloneBranchStateBehind:
		return fmt.Sprintf("behind local %s", targetBranch)
	case git.CloneBranchStateDiverged:
		return fmt.Sprintf("diverged from local %s", targetBranch)
	default:
		return ""
	}
}
...
func (h *Home) deleteSessionWithOptions(inst *session.Instance, forceCloneRemove bool) tea.Cmd {
	id := inst.ID
	isWorktree := inst.IsWorktree()
	isClone := inst.IsClone()
	worktreePath := inst.WorktreePath
	worktreeRepoRoot := inst.WorktreeRepoRoot
	var cloneDeleteErr error
	if !forceCloneRemove {
		cloneDeleteErr = h.cloneDeletionBlockedError(inst)
	}
	return func() tea.Msg {
		if cloneDeleteErr != nil {
			return sessionDeletedMsg{deletedID: id, cleanupErr: cloneDeleteErr}
		}
		...
		if isWorktree {
			...
			if forceCloneRemove {
				archiveResult, archiveErr = git.ArchiveCloneStateIfNeeded(worktreeRepoRoot, worktreePath)
				if archiveErr != nil {
					return sessionDeletedMsg{
						deletedID:  id,
						killErr:    killErr,
						cleanupErr: fmt.Errorf("failed to archive clone state for '%s': %w", inst.Title, archiveErr),
					}
				}
			}
			if err := git.RemoveClone(worktreePath, forceCloneRemove); err != nil {
				return sessionDeletedMsg{
					deletedID:    id,
					killErr:      killErr,
					cleanupErr:   fmt.Errorf("failed to remove clone for '%s': %w", inst.Title, err),
					cloneArchive: archiveResult,
				}
			}
		}
		return sessionDeletedMsg{deletedID: id, killErr: killErr, cloneArchive: archiveResult}
	}
}
...
func (h *Home) updateSizes() {
	...
	h.worktreeFinishDialog.SetSize(h.width, h.height)
	h.cloneCheckpointDialog.SetSize(h.width, h.height)
	h.delegatedCheckpointDialog.SetSize(h.width, h.height)
	h.delegatedFinishDialog.SetSize(h.width, h.height)
	h.repoCloseoutDialog.SetSize(h.width, h.height)
}
...
func (h *Home) View() string {
	...
	if h.cloneCheckpointDialog.IsVisible() {
		return h.cloneCheckpointDialog.View()
	}
	if h.delegatedCheckpointDialog.IsVisible() {
		return h.delegatedCheckpointDialog.View()
	}
	if h.delegatedFinishDialog.IsVisible() {
		return h.delegatedFinishDialog.View()
	}
	if h.repoCloseoutDialog.IsVisible() {
		return h.repoCloseoutDialog.View()
	}
	...
	titleParts := []string{titleStyle.Render("Agent Deck")}
	if label := RuntimeUILabel(); label != "" {
		labelStyle := lipgloss.NewStyle().
			...
			Background(ColorYellow).
			Padding(0, 1)
		titleParts = append(titleParts, labelStyle.Render(label))
	}
	...
	if h.profile != "" && h.profile != session.DefaultProfile {
		profileStyle := lipgloss.NewStyle().
			...
			Bold(true)
		titleParts = append(titleParts, profileStyle.Render("["+h.profile+"]"))
	}
...
func (h *Home) renderHelpBarMinimal() string {
	...
	repoSweepKey := h.actionKey(hotkeyRepoCloseout)
	checkpointKey := h.actionKey(hotkeyCloneCheckpoint)
	...
	if h.jumpMode {
		...
		contextKeys = renderKeys(newKey, quickKey, importKey, repoSweepKey, groupKey)
	} else if h.cursor < len(h.flatItems) {
		item := h.flatItems[h.cursor]
		if item.Type == session.ItemTypeGroup {
			contextKeys = renderKeys("⏎", newKey, quickKey, repoSweepKey, groupKey)
		} else {
			contextKeys = renderKeys("⏎", newKey, quickKey, restartKey, repoSweepKey)
			if item.Session != nil && item.Session.IsClone() {
				checkpointRendered := renderKeys(checkpointKey)
				if checkpointRendered != "" {
					contextKeys += " " + checkpointRendered
				}
			}
...
func (h *Home) renderHelpBarCompact() string {
	...
	if len(h.flatItems) == 0 {
		...
		if repoSweepKey != "" {
			contextHints = append(contextHints, h.helpKeyShort(repoSweepKey, "Sweep"))
		}
	} else if h.cursor < len(h.flatItems) {
		item := h.flatItems[h.cursor]
		if item.Type == session.ItemTypeGroup {
			...
			if item.Session != nil && item.Session.IsClone() && checkpointKey != "" {
				contextHints = append(contextHints, h.helpKeyShort(checkpointKey, "Checkpt"))
			}
			if item.Session != nil && item.Session.CanFork() {
...
func (h *Home) renderHelpBarFull() string {
	...
	if len(h.flatItems) == 0 {
		...
		if repoSweepKey != "" {
			primaryHints = append(primaryHints, h.helpKey(repoSweepKey, "Sweep"))
		}
		if groupKey != "" {
		...
		if item.Type == session.ItemTypeGroup {
			...
			if item.Session != nil && item.Session.IsClone() && checkpointKey != "" {
				primaryHints = append(primaryHints, h.helpKey(checkpointKey, "Checkpoint"))
			}
			primaryHints = append(primaryHints, h.helpKey("t", "Ownership"))
...
func (h *Home) renderSessionItem(
	...
	// Worktree/clone branch badge for isolated sessions.
	worktreeBadge := ""
	if inst.IsIsolated() && inst.WorktreeBranch != "" {
		branch := inst.WorktreeBranch
		if len(branch) > 15 {
			branch = branch[:12] + "..."
		}
	...
	ownershipBadge := ""
	ownershipStyle := OwnershipUserBadgeStyle
	if inst.IsCatoManaged() {
		ownershipStyle = OwnershipCatoBadgeStyle
	}
	if selected {
		ownershipStyle = OwnershipBadgeSelStyle
	}
	ownershipBadge = ownershipStyle.Render(" [" + inst.OwnershipMode().String() + "]")
	...
	// Build row: [baseIndent][selection][tree][chevron][status] [title] [tool] [ownership] [yolo] [worktree] [sandbox] [ssh]
	row := fmt.Sprintf(
...
func (h *Home) renderPreviewPane(width, height int) string {
	...
	b.WriteString("\n")
	b.WriteString(infoStyle.Render("owner: " + selected.OwnershipMode().String()))
	b.WriteString("\n")

	// Worktree/clone info section (for sessions running in isolated workspaces)
	if selected.IsIsolated() {
		sectionName := "Worktree"
		if selected.IsClone() {
			sectionName = "Clone"
		}
		wtHeader := renderSectionDivider(sectionName, width-4)
		...
		cloneState := h.cloneBranchStateCache[selected.ID]
		cloneTargetBranch := h.cloneTargetBranchCache[selected.ID]
		h.worktreeDirtyMu.Unlock()
		...
		if selected.IsClone() && hasCached {
			if syncLabel := describeCloneBranchStateCompact(cloneState, cloneTargetBranch); syncLabel != "" {
				syncStyle := wtValueStyle
				switch cloneState {
				case git.CloneBranchStateInSync:
				...
				b.WriteString(wtLabelStyle.Render("Sync:    "))
		...
		if finishKey := h.actionKey(hotkeyWorktreeFinish); finishKey != "" {
			...
			b.WriteString(wtHintStyle.Render(" local merge + cleanup"))
			b.WriteString("\n")
		}
		if selected.IsClone() {
			if checkpointKey := h.actionKey(hotkeyCloneCheckpoint); checkpointKey != "" {
				b.WriteString(wtHintStyle.Render("Checkpoint: "))
				b.WriteString(wtKeyStyle.Render(checkpointKey))
				b.WriteString(wtHintStyle.Render(" local integrate + resync"))
				b.WriteString("\n")
			}
		}
		if delegatedKey := h.actionKey(hotkeyDelegatedFinish); delegatedKey != "" {
			b.WriteString(wtHintStyle.Render("AI:      "))
			b.WriteString(wtKeyStyle.Render(delegatedKey))
			b.WriteString(wtHintStyle.Render(" spawn closeout agent"))
			b.WriteString("\n")
		}
		if repoSweepKey := h.actionKey(hotkeyRepoCloseout); repoSweepKey != "" {
			b.WriteString(wtHintStyle.Render("Sweep:   "))
			b.WriteString(wtKeyStyle.Render(repoSweepKey))
			b.WriteString(wtHintStyle.Render(" spawn repo sweep agent"))
			b.WriteString("\n")
		}
	}
...
func (h *Home) handleWorktreeFinishDialogKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "confirm":
		...
		mergeEnabled, targetBranch, keepBranch, forceCloneRemove := h.worktreeFinishDialog.GetOptions()
		...
		isClone := h.worktreeFinishDialog.IsCloneSession()
		...
		return h, h.finishWorktree(inst, sid, sTitle, branch, repoRoot, wtPath, mergeEnabled, targetBranch, keepBranch, forceCloneRemove, isClone)
...
func (h *Home) handleDelegatedFinishRequest(msg delegatedFinishRequestMsg) (tea.Model, tea.Cmd) {
	if strings.TrimSpace(msg.sourceSessionID) == "" {
		return h, nil
	}

	h.instancesMu.RLock()
	source := h.instanceByID[msg.sourceSessionID]
	groupPath := ""
	if source != nil {
		groupPath = source.GroupPath
	}
	...
	if source == nil {
		h.setError(fmt.Errorf("source session for AI closeout no longer exists"))
		return h, nil
	}
	if !source.IsWorktree() && !source.IsClone() {
		h.setError(fmt.Errorf("session '%s' is not a worktree or clone", source.Title))
		return h, nil
	}

	repoRoot := strings.TrimSpace(source.WorktreeRepoRoot)
	if repoRoot == "" {
		h.setError(fmt.Errorf("session '%s' has no canonical repo root", source.Title))
		return h, nil
	}

	sessionTitle := h.delegatedFinishSessionTitle(source.Title, groupPath)

	toolOptionsJSON, err := delegatedFinishToolOptions(msg.tool)
	if err != nil {
		h.setError(err)
		return h, nil
	}
...
func (h *Home) handleCloneCheckpointRequest(msg cloneCheckpointRequestMsg) (tea.Model, tea.Cmd) {
	if strings.TrimSpace(msg.sourceSessionID) == "" {
		return h, nil
	}
	...
	if source == nil {
		h.setError(fmt.Errorf("source session for clone checkpoint no longer exists"))
		return h, nil
	}
	if !source.IsClone() {
		h.setError(fmt.Errorf("session '%s' is not a clone session", source.Title))
		return h, nil
	}

	targetBranch := strings.TrimSpace(msg.targetBranch)
	if targetBranch == "" {
		targetBranch, _ = git.GetDefaultBranch(source.WorktreeRepoRoot)
	}
	plan, err := git.PlanCloneCheckpoint(source.WorktreeRepoRoot, source.WorktreePath, source.WorktreeBranch, targetBranch)
	if err != nil {
		h.cloneCheckpointDialog.SetError(err.Error())
		return h, nil
	}
	if plan.CloneDirty {
		h.cloneCheckpointDialog.SetError("clone has uncommitted changes; checkpoint committed work first or use AI checkpoint")
		return h, nil
	}
	if plan.CanonicalRootDirty {
		h.cloneCheckpointDialog.SetError("canonical repo root has uncommitted changes; deterministic checkpoint refuses to trample them. Use AI checkpoint or clean the root first.")
		return h, nil
	}
	if plan.ConflictPredicted {
		errMsg := fmt.Sprintf("deterministic checkpoint predicts a merge conflict against local %s", plan.TargetBranch)
		if strings.TrimSpace(plan.ConflictSummary) != "" {
			errMsg += ": " + plan.ConflictSummary
		}
		errMsg += ". Use AI checkpoint or reconcile manually, then resync."
		h.cloneCheckpointDialog.SetError(errMsg)
		return h, nil
	}
	if plan.Action == git.CloneCheckpointActionUnknown {
		h.cloneCheckpointDialog.SetError("could not determine a truthful deterministic checkpoint action")
		return h, nil
	}

	h.cloneCheckpointDialog.SetExecuting(true)
	return h, h.checkpointClone(source, plan.TargetBranch)
}

func (h *Home) handleOpenDelegatedCheckpointDialog(msg openDelegatedCheckpointDialogMsg) (tea.Model, tea.Cmd) {
	if strings.TrimSpace(msg.sourceSessionID) == "" {
		return h, nil
	}
	...
	if source == nil {
		h.setError(fmt.Errorf("source session for AI checkpoint no longer exists"))
		return h, nil
	}
	if !source.IsClone() {
		h.setError(fmt.Errorf("session '%s' is not a clone session", source.Title))
	...
	h.delegatedCheckpointDialog.SetSize(h.width, h.height)
	h.delegatedCheckpointDialog.Show(
		source.ID,
		source.Title,
		source.WorktreeBranch,
		source.WorktreeRepoRoot,
		source.WorktreePath,
		targetBranch,
	)
	return h, nil
...
func (h *Home) handleDelegatedCheckpointRequest(msg delegatedCheckpointRequestMsg) (tea.Model, tea.Cmd) {
	if strings.TrimSpace(msg.sourceSessionID) == "" {
		return h, nil
	}
	...
	sessionTitle := h.delegatedCheckpointSessionTitle(source.Title, groupPath)
...
func (h *Home) handleRepoCloseoutRequest(msg repoCloseoutRequestMsg) (tea.Model, tea.Cmd) {
	repoRoot, err := h.normalizeRepoCloseoutRoot(msg.repoRoot)
	if err != nil {
		h.setError(fmt.Errorf("repo sweep root %q is not inside a git repo", strings.TrimSpace(msg.repoRoot)))
		return h, nil
	}

	groupPath := strings.TrimSpace(msg.groupPath)
	if groupPath == "" {
		groupPath = session.DefaultGroupPath
	}

	anchorLabel := strings.TrimSpace(msg.anchorLabel)
	anchorSessionID := ""
	if strings.TrimSpace(msg.anchorSessionID) != "" {
		h.instancesMu.RLock()
		source := h.instanceByID[msg.anchorSessionID]
		h.instancesMu.RUnlock()
		if source != nil {
			sourceRepoRoot, sourceErr := h.resolveRepoCloseoutRoot(source)
			if sourceErr == nil && sourceRepoRoot == repoRoot {
				anchorSessionID = source.ID
				anchorLabel = source.Title
				if source.GroupPath != "" {
					groupPath = source.GroupPath
				}
			}
	...
	if anchorLabel == "" {
		anchorLabel = "manual repo sweep"
	}

	sessionTitle := h.repoCloseoutSessionTitle(repoRoot, groupPath)
	trackedSessions := h.collectRepoCloseoutSessions(repoRoot)
	orphanClones := h.collectRepoCloseoutOrphanClones(repoRoot)
	...
	prompt, err := buildRepoCloseoutPrompt(repoCloseoutPromptSpec{
		SweepSessionTitle: sessionTitle,
		AnchorSessionID:   anchorSessionID,
		AnchorSession:     anchorLabel,
		RepoRoot:          repoRoot,
		TargetBranch:      msg.targetBranch,
...
func (h *Home) checkpointClone(inst *session.Instance, targetBranch string) tea.Cmd {
	if inst == nil {
		return nil
	}
	...
	return func() tea.Msg {
		...
		return cloneCheckpointResultMsg{
			sessionID:         sessionID,
			sessionTitle:      sessionTitle,
			targetBranch:      result.TargetBranch,
			branchStateBefore: result.BranchStateBefore,
...
func (h *Home) prepareRepoCloseoutDialogContext() (anchorSessionID, anchorLabel, groupPath, repoRoot string) {
	anchorLabel = "manual repo sweep"
	groupPath = session.DefaultGroupPath

	if h.cursor < 0 || h.cursor >= len(h.flatItems) {
		return anchorSessionID, anchorLabel, groupPath, repoRoot
	}
	...
	case session.ItemTypeSession:
		...
		inst := item.Session
		anchorSessionID = inst.ID
		anchorLabel = inst.Title
		if inst.GroupPath != "" {
			groupPath = inst.GroupPath
		}
		if resolvedRoot, err := h.resolveRepoCloseoutRoot(inst); err == nil {
	...
	case session.ItemTypeWindow:
		...
		h.instancesMu.RLock()
		inst := h.instanceByID[item.WindowSessionID]
		h.instancesMu.RUnlock()
		if inst == nil {
			return anchorSessionID, anchorLabel, groupPath, repoRoot
		}
	...
	case session.ItemTypeGroup:
		...
		anchorLabel = item.Group.Name
		if item.Group.Path != "" {
			groupPath = item.Group.Path
		}
		repoRoot = h.suggestRepoCloseoutRootForGroup(item.Group)
	case session.ItemTypeRemoteSession:
		if item.RemoteSession != nil {
			anchorLabel = item.RemoteSession.Title
		}
	case session.ItemTypeRemoteGroup:
		if item.RemoteSession != nil && item.RemoteSession.Title != "" {
			anchorLabel = item.RemoteSession.Title
		} else if strings.TrimSpace(item.RemoteName) != "" {
			anchorLabel = item.RemoteName
		}
	}
...
func (h *Home) resolveRepoCloseoutRoot(source *session.Instance) (string, error) {
	if source == nil {
		return "", fmt.Errorf("no session selected for repo sweep")
	}

	if repoRoot := strings.TrimSpace(source.WorktreeRepoRoot); repoRoot != "" {
		return repoRoot, nil
	}

	projectPath := strings.TrimSpace(source.ProjectPath)
	if projectPath == "" {
		return "", fmt.Errorf("session '%s' has no project path", source.Title)
	}

	repoRoot, err := git.GetWorktreeBaseRoot(projectPath)
	if err != nil || strings.TrimSpace(repoRoot) == "" {
		return "", fmt.Errorf("session '%s' is not attached to a git repo root", source.Title)
	}
	return repoRoot, nil
}

func (h *Home) normalizeRepoCloseoutRoot(path string) (string, error) {
...
func (h *Home) suggestRepoCloseoutRootForGroup(group *session.Group) string {
	if group == nil {
		return ""
	}

	if root, err := h.normalizeRepoCloseoutRoot(group.DefaultPath); err == nil {
		return root
	}
	...
	for _, inst := range group.Sessions {
		...
		root, err := h.resolveRepoCloseoutRoot(inst)
		if err != nil || root == "" {
			continue
		}
		seen[root] = true
...
func (h *Home) collectRepoCloseoutSessions(repoRoot string) []repoCloseoutTrackedSession {
	...
	var tracked []repoCloseoutTrackedSession
	for _, inst := range h.instances {
		if inst == nil || (!inst.IsWorktree() && !inst.IsClone()) {
			continue
		}
		if strings.TrimSpace(inst.WorktreeRepoRoot) != repoRoot {
			continue
		}
		...
		tracked = append(tracked, repoCloseoutTrackedSession{
			ID:            inst.ID,
			Title:         inst.Title,
	...
	sort.Slice(tracked, func(i, j int) bool {
		if tracked[i].IsolationType == tracked[j].IsolationType {
			return tracked[i].Title < tracked[j].Title
		}
		return tracked[i].IsolationType < tracked[j].IsolationType
	})

	return tracked
}

func (h *Home) collectRepoCloseoutOrphanClones(repoRoot string) []git.CloneInfo {
	repoRoot = strings.TrimSpace(repoRoot)
	if repoRoot == "" {
		return nil
	}
	...
	for _, tracked := range h.collectRepoCloseoutSessions(repoRoot) {
		if tracked.IsolationType == "clone" {
			trackedPaths[tracked.Path] = true
		}
	}

	orphans := make([]git.CloneInfo, 0, len(clones))
	for _, clone := range clones {
		if trackedPaths[clone.Path] {
			continue
		}
		orphans = append(orphans, clone)
	}
...
func delegatedFinishToolOptions(tool string) (json.RawMessage, error) {
	switch strings.TrimSpace(tool) {
	case "codex":
		userConfig, _ := session.LoadUserConfig()
		opts := session.NewCodexOptions(userConfig)
		opts.Model = delegatedFinishDefaultCodexModel
		return session.MarshalToolOptions(opts)
	default:
		return nil, nil
	}
}

func (h *Home) repoCloseoutSessionTitle(repoRoot, groupPath string) string {
	baseName := filepath.Base(strings.TrimRight(strings.TrimSpace(repoRoot), string(os.PathSeparator)))
	if baseName == "" || baseName == "." || baseName == string(os.PathSeparator) {
		baseName = "repo"
	}
	base := truncateRunes("repo-closeout: "+baseName, MaxNameLength)

	title := base
	suffix := 2
	for h.sessionTitleExistsInGroup(title, groupPath) {
		suffixText := fmt.Sprintf(" (%d)", suffix)
		title = truncateRunes(base, MaxNameLength-len([]rune(suffixText))) + suffixText
		suffix++
	}
	return title
}

func (h *Home) delegatedFinishSessionTitle(sourceTitle, groupPath string) string {
	base := strings.TrimSpace(sourceTitle)
	if base == "" {
		base = "session"
	}
	base = truncateRunes("closeout: "+base, MaxNameLength)
...
func (h *Home) delegatedCheckpointSessionTitle(sourceTitle, groupPath string) string {
	base := strings.TrimSpace(sourceTitle)
	if base == "" {
		base = "session"
	}
	base = truncateRunes("checkpoint: "+base, MaxNameLength)
...
func (h *Home) sessionTitleExistsInGroup(title, groupPath string) bool {
	h.instancesMu.RLock()
	defer h.instancesMu.RUnlock()
	for _, inst := range h.instances {
		if inst != nil && inst.GroupPath == groupPath && inst.Title == title {
			return true
		}
	}
	return false
...
func (h *Home) sendInitialMessageToSession(inst *session.Instance, message string) tea.Cmd {
	if inst == nil || strings.TrimSpace(message) == "" {
		return nil
	}
	...
	return func() tea.Msg {
		err := session.SendSessionMessageReliable(profile, sessionID, message)
		return sessionMessageSentMsg{
			sessionID:    sessionID,
			sessionTitle: sessionTitle,
			err:          err,
		}
...
// finishWorktree performs the finish operation asynchronously for worktree or
// clone sessions: local merge, workspace cleanup, optional branch retention,
// tmux kill, and session removal from storage.
func (h *Home) finishWorktree(inst *session.Instance, sessionID, sessionTitle, branchName, repoRoot, worktreePath string, mergeEnabled bool, targetBranch string, keepBranch bool, forceCloneRemove bool, isClone bool) tea.Cmd {
	return func() tea.Msg {
		merged := false
		archiveResult := git.CloneArchiveResult{}
		var archiveErr error
		...
		if mergeEnabled {
			if isClone {
				if err := git.MergeCloneBranch(repoRoot, worktreePath, branchName, targetBranch); err != nil {
					return worktreeFinishResultMsg{
						sessionID: sessionID, sessionTitle: sessionTitle, isClone: isClone,
						err: fmt.Errorf("local clone merge failed: %w", err),
					}
				}
			} else {
		...
		// Step 2: Retain clone branch locally when requested.
		if isClone && keepBranch {
			if err := git.RetainCloneBranchLocally(repoRoot, worktreePath, branchName); err != nil {
				return worktreeFinishResultMsg{
					sessionID: sessionID, sessionTitle: sessionTitle, isClone: isClone,
					err: err,
				}
		...
		if _, statErr := os.Stat(worktreePath); !os.IsNotExist(statErr) {
			if isClone {
				if forceCloneRemove && !merged {
					archiveResult, archiveErr = git.ArchiveCloneStateIfNeeded(repoRoot, worktreePath)
					if archiveErr != nil {
						return worktreeFinishResultMsg{
							sessionID: sessionID, sessionTitle: sessionTitle, isClone: isClone,
							err: fmt.Errorf("failed to archive clone state before forced finish cleanup: %w", archiveErr),
						}
					}
				}
				if err := git.RemoveClone(worktreePath, forceCloneRemove); err != nil {
			...
			} else if err := git.RemoveWorktree(repoRoot, worktreePath, false); err != nil {
				return worktreeFinishResultMsg{
					sessionID: sessionID, sessionTitle: sessionTitle, isClone: isClone,
					err: err,
				}
		...
		// Step 4: Delete local branch (if not keeping and it exists in repoRoot)
		if !keepBranch && git.BranchExists(repoRoot, branchName) {
			// Use force delete if we merged or the operator explicitly forced clone cleanup.
			_ = git.DeleteBranch(repoRoot, branchName, merged || forceCloneRemove)
		}
