...
func TestNewDialog_GetValuesWithIsolation_Worktree(t *testing.T) {
	dialog := NewNewDialog()
	dialog.worktreeEnabled = true
	...
	name, path, command, branch, cloneName, worktreeEnabled, cloneEnabled := dialog.GetValuesWithIsolation()

	if !worktreeEnabled {
		t.Error("worktreeEnabled should be true")
	}
	if cloneEnabled {
		t.Error("cloneEnabled should be false")
	}
	if branch != "feature/test" {
		t.Errorf("Branch: got %q, want %q", branch, "feature/test")
	}
	if cloneName != "" {
		t.Errorf("Clone name: got %q, want empty", cloneName)
	}
...
func TestNewDialog_GetValuesWithIsolation_Clone(t *testing.T) {
	dialog := NewNewDialog()
	dialog.cloneEnabled = true
	dialog.cloneNameInput.SetValue("agent-reviewer")
	dialog.nameInput.SetValue("test-session")
	dialog.pathInput.SetValue("/tmp/project")
	...
	if !cloneEnabled {
		t.Error("cloneEnabled should be true")
	}
	if branch != "" {
		t.Errorf("Branch: got %q, want empty", branch)
	}
	if cloneName != "agent-reviewer" {
		t.Errorf("Clone name: got %q, want %q", cloneName, "agent-reviewer")
	}
	if name != "test-session" {
		t.Errorf("Name: got %q, want %q", name, "test-session")
	}
...
func TestNewDialog_GetValuesWithIsolation_Disabled(t *testing.T) {
	dialog := NewNewDialog()
	dialog.worktreeEnabled = false
	dialog.cloneEnabled = false
	dialog.branchInput.SetValue("feature/test")
	dialog.cloneNameInput.SetValue("agent-reviewer")

	_, _, _, branch, cloneName, worktreeEnabled, cloneEnabled := dialog.GetValuesWithIsolation()
...
func TestNewDialog_Validate_CloneEnabled_EmptyCloneName(t *testing.T) {
	dialog := NewNewDialog()
	dialog.nameInput.SetValue("test-session")
	dialog.pathInput.SetValue("/tmp/project")
	dialog.cloneEnabled = true
	dialog.cloneNameInput.SetValue("")

	err := dialog.Validate()
	if err == "" {
		t.Error("Validation should fail when clone enabled but clone name is empty")
	}
	if err != "Clone name required for clone" {
		t.Errorf("Unexpected error message: %q", err)
	}
}

func TestNewDialog_Validate_CloneEnabled_InvalidCloneName(t *testing.T) {
	dialog := NewNewDialog()
	dialog.nameInput.SetValue("test-session")
	dialog.pathInput.SetValue("/tmp/project")
	dialog.cloneEnabled = true
	dialog.cloneNameInput.SetValue("bad/name")

	err := dialog.Validate()
	if err == "" {
		t.Error("Validation should fail for invalid clone name")
	}
	if err != "clone name cannot contain path separators" {
		t.Errorf("Unexpected error message: %q", err)
	}
}
...
func TestNewDialog_View_ShowsCloneCheckbox(t *testing.T) {
	dialog := NewNewDialog()
	dialog.SetSize(80, 40)
	dialog.Show()
	...
	if !strings.Contains(view, "Create reference clone") {
		t.Error("View should contain 'Create reference clone' checkbox")
	}
	if !strings.Contains(view, "(c)") {
		t.Error("View should contain '(c)' hint when on command field")
	}
}
...
func TestNewDialog_View_ShowsCloneNameWhenEnabled(t *testing.T) {
	dialog := NewNewDialog()
	dialog.SetSize(80, 40)
	dialog.Show()
	dialog.cloneEnabled = true
	...
	if !strings.Contains(view, "Clone Name:") {
		t.Error("View should contain 'Clone Name:' label when clone enabled")
	}
	if !strings.Contains(view, "[x]") {
		t.Error("View should show checked checkbox [x] when clone enabled")
	}
}
...
func TestNewDialog_CloneCheckbox_SpaceToggle(t *testing.T) {
	dialog := NewNewDialog()
	dialog.Show()
	dialog.sandboxEnabled = false
	dialog.inheritedSettings = nil
	dialog.rebuildFocusTargets()
	dialog.focusIndex = dialog.indexOf(focusClone)

	dialog, _ = dialog.Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{' '}})

	if !dialog.cloneEnabled {
		t.Error("Space on clone checkbox should enable clone mode")
	}
	if dialog.focusIndex != dialog.indexOf(focusCloneName) {
		t.Errorf("Focus should move to clone name field (%d), got %d", dialog.indexOf(focusCloneName), dialog.focusIndex)
	}
	...
	if dialog.cloneEnabled {
		t.Error("Space on clone checkbox should disable clone mode")
	}
}
...
func TestNewDialog_CheckboxesFocusIndependently(t *testing.T) {
	...
	// Focus on clone checkbox.
	dialog.focusIndex = dialog.indexOf(focusClone)
	view = dialog.View()

	if !strings.Contains(view, "Create reference clone") {
		t.Error("View should contain clone checkbox")
	}
...
func TestNewDialog_ToggleClone_AutoPopulatesCloneName(t *testing.T) {
	d := NewNewDialog()
	d.nameInput.SetValue("Amber Falcon (Review)")

	d.ToggleClone()

	if !d.cloneEnabled {
		t.Fatal("cloneEnabled should be true after toggle")
	}
	if d.cloneNameInput.Value() != "amber-falcon-review" {
		t.Errorf("clone name = %q, want %q", d.cloneNameInput.Value(), "amber-falcon-review")
	}
	if !d.cloneAutoSet {
		t.Error("cloneAutoSet should be true after auto-population")
	}
}

func TestNewDialog_ToggleClone_DisablesWorktree(t *testing.T) {
	d := NewNewDialog()
	d.worktreeEnabled = true
	d.branchInput.SetValue("feature/test")
	d.branchAutoSet = true

	d.ToggleClone()

	if d.worktreeEnabled {
		t.Error("worktree should be disabled when clone is enabled")
	}
	if d.branchInput.Value() != "" {
		t.Errorf("branch should be cleared when clone is enabled, got %q", d.branchInput.Value())
	}
}
