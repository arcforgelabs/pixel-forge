...
const (
	...
	focusCloneName             // clone name input (conditional — only when clone enabled).
...
type NewDialog struct {
	...
	cloneAutoSet   bool // true if clone name was auto-derived from session name.
...
func NewNewDialog() *NewDialog {
	...
	cloneNameInput.Placeholder = "agent-name"
	cloneNameInput.CharLimit = 100
	cloneNameInput.Width = 40
...
func (d *NewDialog) ShowInGroup(groupPath, groupName, defaultPath string) {
	...
	// Reset clone fields.
	d.cloneEnabled = false
	d.cloneNameInput.SetValue("")
	d.cloneAutoSet = false
...
func (d *NewDialog) saveSnapshot() *dialogSnapshot {
	...
	return &dialogSnapshot{
		...
		cloneEnabled:    d.cloneEnabled,
		cloneName:       d.cloneNameInput.Value(),
		cloneAutoSet:    d.cloneAutoSet,
...
func (d *NewDialog) restoreSnapshot(s *dialogSnapshot) {
	...
	d.cloneEnabled = s.cloneEnabled
	d.cloneNameInput.SetValue(s.cloneName)
	d.cloneAutoSet = s.cloneAutoSet
...
// ToggleClone toggles the clone checkbox.
// When enabling, auto-populates the clone name from the session name.
func (d *NewDialog) ToggleClone() {
	d.cloneEnabled = !d.cloneEnabled
	if d.cloneEnabled {
		d.worktreeEnabled = false
		d.branchInput.SetValue("")
		d.branchAutoSet = false
		d.autoCloneNameFromName()
	}
	d.rebuildFocusTargets()
}

func (d *NewDialog) autoCloneNameFromName() {
	name := sanitizeCloneName(strings.TrimSpace(d.nameInput.Value()))
	if name == "" {
		return
	}
	d.cloneNameInput.SetValue(name)
	d.cloneAutoSet = true
}

func sanitizeCloneName(name string) string {
...
// IsCloneEnabled returns whether clone mode is enabled.
func (d *NewDialog) IsCloneEnabled() bool {
	return d.cloneEnabled
}

// GetValuesWithIsolation returns all values including isolation settings.
func (d *NewDialog) GetValuesWithIsolation() (name, path, command, branch, cloneName string, worktreeEnabled, cloneEnabled bool) {
	name, path, command = d.GetValues()
	branch = strings.TrimSpace(d.branchInput.Value())
	cloneName = strings.TrimSpace(d.cloneNameInput.Value())
	worktreeEnabled = d.worktreeEnabled
	cloneEnabled = d.cloneEnabled
	return
}
...
func (d *NewDialog) Validate() string {
	...
	if d.worktreeEnabled && d.cloneEnabled {
		return "Choose either worktree or clone"
	}
	...
	if d.cloneEnabled {
		cloneName := strings.TrimSpace(d.cloneNameInput.Value())
		if cloneName == "" {
			return "Clone name required for clone"
		}
		if err := git.ValidateCloneName(cloneName); err != nil {
			return err.Error()
		}
	}
...
func (d *NewDialog) rebuildFocusTargets() {
	targets := []focusTarget{focusName, focusPath, focusCommand, focusWorktree, focusClone, focusSandbox}
	...
	if d.cloneEnabled {
		targets = append(targets, focusCloneName)
	}
...
func (d *NewDialog) updateFocus() {
	...
	case focusWorktree, focusClone, focusSandbox, focusInherited:
		// Checkbox/toggle rows — no text input to focus.
	case focusBranch:
		d.branchInput.Focus()
	case focusCloneName:
		d.cloneNameInput.Focus()
	case focusOptions:
		if d.toolOptions != nil {
			d.toolOptions.Focus()
		}
...
func (d *NewDialog) Update(msg tea.Msg) (*NewDialog, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "c":
			if cur == focusCommand {
				d.ToggleClone()
				if d.cloneEnabled {
					if idx := d.indexOf(focusCloneName); idx >= 0 {
						d.focusIndex = idx
					}
					d.updateFocus()
				}
	...
	case focusName:
		...
		if d.cloneEnabled && d.cloneAutoSet && d.nameInput.Value() != oldName {
			d.autoCloneNameFromName()
		}
	case focusPath:
	...
	case focusCloneName:
		oldCloneName := d.cloneNameInput.Value()
		d.cloneNameInput, cmd = d.cloneNameInput.Update(msg)
		if d.cloneNameInput.Value() != oldCloneName {
			d.cloneAutoSet = false
		}
	case focusOptions:
		if d.toolOptions != nil {
...
func (d *NewDialog) View() string {
	...
	// Clone checkbox — individually focusable.
	cloneLabel := "Create reference clone"
	if cur == focusCommand {
		cloneLabel = "Create reference clone (c)"
	}
	content.WriteString(renderCheckboxLine(cloneLabel, d.cloneEnabled, cur == focusClone))
	...
	// Clone name input (only visible when clone is enabled).
	if d.cloneEnabled {
		content.WriteString("\n")
		if cur == focusCloneName {
			content.WriteString(activeLabelStyle.Render("▶ Clone Name:"))
		} else {
			content.WriteString(labelStyle.Render("  Clone Name:"))
		}
		content.WriteString("\n")
		content.WriteString("  ")
		content.WriteString(d.cloneNameInput.View())
		content.WriteString("\n")
	}
