...
func (h *HelpOverlay) View() string {
	...
	checkpointKey := h.key(hotkeyCloneCheckpoint, "I")
	delegatedFinishKey := h.key(hotkeyDelegatedFinish, "A")
	repoCloseoutKey := h.key(hotkeyRepoCloseout, "B")
	...
	sections := []struct {
		...
		},
		{
			title: "ISOLATION",
			items: [][2]string{
				{worktreeKey, "Finish worktree/clone (local merge + cleanup)"},
				{checkpointKey, "Checkpoint clone (local integrate + resync, AI option in dialog)"},
				{delegatedFinishKey, "AI closeout for one worktree/clone"},
				{repoCloseoutKey, "AI repo sweep for all worktrees/clones under repo"},
				{"n → w / c", "Create session in worktree/clone"},
	...
	lines = append(lines, versionStyle.Render(RuntimeDisplayName()+" v"+Version))
