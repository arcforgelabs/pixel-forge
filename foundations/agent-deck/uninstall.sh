...
BINARY_LOCATIONS=(
    ...
    "$HOME/.local/bin/agent-deck-stock"
    "$HOME/.local/bin/agent-deck-layers"
...
# Check for layered runtime directory
LAYERS_DIR="$HOME/.local/share/agent-deck/layers"
if [[ -d "$LAYERS_DIR" ]]; then
    FOUND_ITEMS+=("layers")
    LAYERS_SIZE=$(du -sh "$LAYERS_DIR" 2>/dev/null | cut -f1)
    echo -e "Found: ${GREEN}Layered runtime${NC} at $LAYERS_DIR"
    echo -e "       ${DIM}$LAYERS_SIZE total${NC}"
fi

# Check for upstream stock sandbox
STOCK_SANDBOX_DIR="$HOME/.local/share/agent-deck/sandboxes/upstream-stock"
if [[ -d "$STOCK_SANDBOX_DIR" ]]; then
    FOUND_ITEMS+=("stock-sandbox")
    STOCK_SANDBOX_SIZE=$(du -sh "$STOCK_SANDBOX_DIR" 2>/dev/null | cut -f1)
    echo -e "Found: ${GREEN}Upstream stock sandbox${NC} at $STOCK_SANDBOX_DIR"
    echo -e "       ${DIM}$STOCK_SANDBOX_SIZE total${NC}"
fi
...
for item in "${FOUND_ITEMS[@]}"; do
    case "$item" in
        ...
        data)
            ...
            ;;
        layers)
            echo -e "  ${RED}•${NC} Layered runtime: $LAYERS_DIR"
            ;;
        stock-sandbox)
            echo -e "  ${RED}•${NC} Upstream stock sandbox: $STOCK_SANDBOX_DIR"
            ;;
        tmux)
...
# 2b. Layered runtime directory
if [[ " ${FOUND_ITEMS[*]} " =~ " layers " ]]; then
    echo -e "Removing layered runtime at $LAYERS_DIR..."
    rm -rf "$LAYERS_DIR"
    echo -e "${GREEN}✓${NC} Layered runtime removed"
fi

# 2c. Upstream stock sandbox
if [[ " ${FOUND_ITEMS[*]} " =~ " stock-sandbox " ]]; then
    echo -e "Removing upstream stock sandbox at $STOCK_SANDBOX_DIR..."
    rm -rf "$STOCK_SANDBOX_DIR"
    echo -e "${GREEN}✓${NC} Upstream stock sandbox removed"
fi
