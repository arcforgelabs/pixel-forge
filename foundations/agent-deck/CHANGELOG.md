...
- Fix duplicate clipboard writes from the recommended tmux drag-to-copy configuration in clipboard-capable terminals (for example Ghostty) by disabling tmux OSC 52 clipboard export and routing copy-mode copies through the configured external clipboard command.
- Reduce dashboard navigation/input lag after idle periods by backing off background all-session status sweeps while the main TUI dashboard is visible.
- Auto-resolve conductor escalation records when a child session transitions from `waiting` to `running` (unblocked), so stale `sent`/`failed` alerts are closed without manual `[RESOLVE]`.
- Add escalation TTL expiry: failed/deferred escalation records older than 24 hours are now auto-marked as `expired`, preventing indefinite retry/report churn.
- Update operational cron guidance for `escalation-check` to use an Adelaide-time waking-hours cron window (`0 8-23/2 * * * @ Australia/Adelaide`) instead of 24/7 30-minute polling.

### Added

- Add a side-by-side `upstream-stock` runtime lane, `agent-deck-stock` launcher, and isolated sandbox workflow for comparing pure upstream Agent Deck against the custom overlay without repointing the live launcher.

### Changed

- Align the local fork/runtime workflow with the layered OpenClaw-style model: custom code stays on `custom/samuel-agent-deck`, `./dev-install.sh` publishes the `workstation` lane, `agent-deck-layers` manages runtime lanes, and the old `fork-sync-reinstall.sh` path is replaced by `fork-sync-publish.sh`.
